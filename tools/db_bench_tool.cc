...
DEFINE_double(gc_multiple, 1.0,
            "计算因为空间放大和范围查询选择index时的score值的比值，用来确定其权重");

DEFINE_double(gc_garbage_threshold, 0.30,
            "为了减缓空间放大，选择gc table时有效key的占比阈值");

DEFINE_int32(gc_range_tables_threshold, 10,
            "为了优化范围查询，选择gc index时其引用的table的最小阈值");

DEFINE_double(pick_table_threshold, 0.70,
            "选择gc index时对应table的占比大小");

DEFINE_double(pick_table_max_multiple, 2.0,
            "选择gc index时连续选择table的最大差距");

DEFINE_bool(enable_vertical_compaction,true,
            "表明是否开启vertical compaction");
DEFINE_double(vc_merge_threshold,0.5,
            "为了加速Merge查询，选择vc compaction涉及文件时，文件的最低merge数量阈值");

DEFINE_int32(vertical_start_level, 2,
            "vertical compaction选择的开始level");

DEFINE_int32(vertical_min_involve_levels, 2,
            "vertical compaction的最小包含level");

DEFINE_uint32(pick_index_min_table_numbers, 3,
            "选择gc value compaction时，指向最多table的index的最低指向值。如果低于该值，本次不触发value compaction");

DEFINE_uint64(choose_guard_interval, 2000,
            "选择guard时的差距间隔");

DEFINE_uint32(neighbor_guard_ratio, 3,
            "选择guard时的差距间隔");
#ifndef ROCKSDB_LITE
...
// Current the following isn't equivalent to OS_LINUX.
 ...
 private:
  ...
  void InitializeOptionsFromFlags(Options* opts) {
    ...
    options.gc_multiple = FLAGS_gc_multiple;
    options.gc_garbage_threshold = FLAGS_gc_garbage_threshold;
    options.gc_range_tables_threshold = FLAGS_gc_range_tables_threshold;
    options.pick_table_threshold = FLAGS_pick_table_threshold;
    options.pick_table_max_multiple = FLAGS_pick_table_max_multiple;
    options.enable_vertical_compaction = FLAGS_enable_vertical_compaction;
    options.vc_merge_threshold = FLAGS_vc_merge_threshold;
    options.vertical_start_level = FLAGS_vertical_start_level;
    options.vertical_min_involve_levels = FLAGS_vertical_min_involve_levels;
    options.pick_index_min_table_numbers = FLAGS_pick_index_min_table_numbers;
    options.choose_guard_interval = FLAGS_choose_guard_interval;
    options.neighbor_guard_ratio = FLAGS_neighbor_guard_ratio;
    ...
    if (FLAGS_use_plain_table) {
      ...
      options.table_factory.reset(new NvmPartitionFactory(block_based_options));
      options.second_table_factory.reset(new NvmTableFactory(block_based_options));
    }
