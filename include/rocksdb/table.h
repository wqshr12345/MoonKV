...
struct EnvOptions;
struct table_information_collect {
  uint32_t table_file_number;
  uint32_t valid_key_num;
  uint32_t table_file_size;
};
...
extern TableFactory* NewNvmPartitionFactory(
    const BlockBasedTableOptions& table_options = BlockBasedTableOptions());

extern TableFactory* NewNvmTableFactory(
    const BlockBasedTableOptions& table_options = BlockBasedTableOptions());
...
class TableFactory : public Customizable {
 public:
  ...
  static const char* kNvmTableName() { return "NvmTable"; };
  static const char* kNvmPartitionName() { return "NvmPartition"; };
  ...
  // 仅在NvmPartitionFactory中有效 其它Factory默认使用父函数的实现 返回nullptr
  virtual TableBuilder* NewTableBuilder(
      const TableBuilderOptions& table_builder_options,
      WritableFileWriter* file, uint64_t table_file_seq,
      uint64_t index_file_seq, std::vector<table_information_collect>& sub_run,
