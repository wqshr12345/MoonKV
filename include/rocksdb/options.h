...
struct ColumnFamilyOptions : public AdvancedColumnFamilyOptions {
  ...
  // 设置后台最多同时进行的compaction的作业的个数，避免过多的gc compaction使得整个LSM-Tree变成了一个无序状态
  // Default: 5
  int max_gc_compactions = 5;

  // 计算因为空间放大和范围查询选择index时的score值的比值，用来确定其权重.
  // Default: 1.0
  double gc_multiple = 1.0;

  // 为了减缓空间放大，选择gc table时有效key的占比阈值
  // Default: 0.30.
  double gc_garbage_threshold = 0.30;

  // 为了优化范围查询，选择gc index时其引用的table的最小阈值
  int gc_range_tables_threshold = 10;

  // 选择gc index时对应table的占比大小(用于优化范围查询)
  // Default:0.70
  double pick_table_threshold = 0.70;

  // 选择gc index时连续选择table的最大差距(用于优化范围查询)
  // 本参数应该始终大于1.0
  // Default: 2.0
  double pick_table_max_multiple = 2.0;

  // 表明是否开启vertical compaction
  // Default: true
  bool enable_vertical_compaction = true;


  // 为了加速Merge查询，选择vc compaction涉及文件时，文件的最低merge数量阈值
  double vc_merge_threshold = 0.5;

  double vc_merge_threshold_nostart = 0.1; // 非start level的最低merge数量阈值

  // vertical compaction选择的开始level
  // Default: 2
  int vertical_start_level = 2;
  
  // vertical compaction的最小包含level
  // Default: 2
  int vertical_min_involve_levels = 2;

  // 选择gc index时触发compaction的最少table个数(用于优化范围查询)
  // 优化空间放大而触发的compaction不受此参数影响
  // Default: 3
  uint32_t pick_index_min_table_numbers = 3;

  // 选择guard时的差距间隔
  // Default: 2000
  uint64_t choose_guard_interval = 2000;

  // 相邻guard之间的数量比例
  // Default: 10
  uint32_t neighbor_guard_ratio = 3;

  // 是否开启EstimateSeek
  // Default: true
  bool enable_estimate_seek = true;

  // 开启estimateseek时，构造相邻两层的查询关系时 father iterator的间隔
  int estimate_interval = 100;

  // 开启estiamte_seek时，构造相邻两层的查询关系时 超过此阈值才会添加新的segemnt
  int estimate_threshold = 5;
  ...
  std::shared_ptr<TableFactory> second_table_factory;
...
struct DBOptions {
  ...
  // 添加一个记录NVM文件路径的变量，还需要补充db_bench和YCSB相关配置的逻辑
  std::string db_nvm_dir = "";
...
struct ReadOptions {
  ...
  // block大小
  // 注意，我们之所以要在ReadOptions中加入这个字段是因为我们的设计中没有index block，所以给定offset之后，判断
  // 其在哪个block，严重依赖于block_size
