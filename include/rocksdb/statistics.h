...
enum Tickers : uint32_t {
  ...
  BLOCKS_NO_ESTIMATE,
  BLOCKS_WITH_ESTIMATE,
  NUM_SEARCH_KEYS,
  NUM_SEARCH_FILES,
  NUM_NOT_FOUND, 
  NUM_FOUND, 
  NUM_MERGE,  
  NUM_SEEK_MERGE_KEY_TIMES, // 标识所有的查询有多少次查到了Merge Value
  NUM_SEEK_FULL_KEY_TIMES, // 标识所有的查询有多少次仅查到了Full Value
  NUM_SEEK_MERGE_KEYS_INCLUDE_RELATED_FULL_KEYS, // 标识所有的查询总共查了多少个Merge Value和Full Value
  NUM_SEEK_LEVELS,
  NUM_SEEK_TIMES,
...
enum Histograms : uint32_t {
  ...
  DB_POSITION_GET,
  DB_ITER_NEXT_GET,
  DB_CHILDREN_RANK_GET,
  DB_INDEX_GET, // 从b+tree index获得数据的时间
  DB_INDEX_UNES_GET, // 没开启estimate seek，查询的次数和统计时间
  DB_INDEX_UNES_PARTIAL_GET, //没开启estimate seek，查询前几层时使用到了estimate seek的次数和时间统计
  DB_INDEX_ENES_NOES_GET, // 开启了estimate seek，但查询index未使用到estimate seek的次数和时间统计
  DB_INDEX_ENES_WITHES_GET, // 开启了estimate seek，同时查询index使用到了estimate seek的次数和时间统计
  DB_INDEX_ENES_WITHES_PARTIAL_GET, // 开启了estimate seek，同时查询前几层index使用到了estimate seek的次数和时间统计
  DB_INDEX_KEY_POS_LIST_GET, //开启了estimate seek，从hashmap获取key pos list的次数和时间统计
  DB_INDEX_KEY_POS_GET, // 开启了estimate seek，从keyposlist获取position的时间
  DB_INDEX_ADD_KEY_POS, // 往KeyPositionList添加数据的次数和时间统计
  DB_INDEX_ADD_KEY_POS_LIST, // 往hashmap添加KeyPositionList的次数和时间统计
  DB_INDEX_SEEK1_GET, // 本次实际查询了1个block的时间
  DB_INDEX_SEEK2_GET, // 本次实际查询了2个block的时间
  DB_INDEX_SEEK3_GET,
  DB_INDEX_SEEK4_GET,
  DB_INDEX_SEEK5_GET,
  DB_INDEX_SEEK6_GET,
  DB_INDEX_SEEK7_GET,
  DB_INDEX_SEEK8_GET,
  DB_INDEX_SEEK9_AND_UP_GET,
  DB_INDEX_LEVEL1_GET, // 本次查询的btree有1个level的查询时间
  DB_INDEX_LEVEL2_GET,
  DB_INDEX_LEVEL3_GET,
  DB_INDEX_LEVEL4_GET,
  DB_INDEX_LEVEL5_GET,
  ...
  DB_INDEX_LEVEL7_GET,
  DB_INDEX_LEVEL8_GET,
  DB_INDEX_LEVEL9_AND_UP_GET,
  DB_INDEX_LEVEL1_SEEK_BLOCKS, // b+tree只有1层的情况下，实际查询的block数量 下面以此类推
  DB_INDEX_LEVEL2_SEEK_BLOCKS,
  DB_INDEX_LEVEL3_SEEK_BLOCKS,
  DB_INDEX_LEVEL4_SEEK_BLOCKS,
  DB_INDEX_LEVEL5_SEEK_BLOCKS,
  DB_INDEX_LEVEL6_SEEK_BLOCKS,
  DB_INDEX_LEVEL7_SEEK_BLOCKS,
  DB_INDEX_LEVEL8_SEEK_BLOCKS,
  DB_INDEX_LEVEL9_AND_UP_SEEK_BLOCKS,
  DB_DATA_GET,
  DB_DATA_CACHE_GET, // 获取实际value时 从cache获得数据的时间
  DB_DATA_DISK_GET,  // 获取实际value时 从disk获得数据的时间
  DB_DATA_REAL_DISK_GET,
  DB_GET_FULL_KEY, // 本次获取key 只查到了一个full value 总时间统计
  DB_GET_MERGE_KEY_1, // 本次获取key 只查到了1个merge value 总时间统计
  DB_GET_MERGE_KEY_2, // 本次获取key 查询了lsm-tree的两个merge value 总时间统计
  DB_GET_MERGE_KEY_3, // 本次获取key 查询了lsm-tree的三个merge value 总时间统计
  DB_GET_MERGE_KEY_4, // 本次获取key 查询了lsm-tree的四个merge value 总时间统计
  DB_GET_MERGE_KEY_5_up,  // 本次获取key 查询了lsm-tree的五个以上merge value 总时间统计
  DB_GET_FULL_KEY_NOL0,
  DB_GET_MERGE_KEY_1_NOL0, 
  DB_GET_MERGE_KEY_2_NOL0, 
  DB_GET_MERGE_KEY_3_NOL0, 
  DB_GET_MERGE_KEY_4_NOL0, 
  DB_GET_MERGE_KEY_5_up_NOL0,
  DB_GET_LEVEL_1,
  DB_GET_LEVEL_2, // 本次获取key 查询了lsm-tree的两层 总时间统计
  DB_GET_LEVEL_3, // 本次获取key 查询了lsm-tree的三层 总时间统计
  DB_GET_LEVEL_4, // 本次获取key 查询了lsm-tree的四层 总时间统计
  DB_GET_LEVEL_5_up, // 本次获取key 查询了lsm-tree的五层以上 总时间统计
  DB_SEEK_1_TIMES, // 这里和DB_GET_FULL_KEY那一系列类似 区别在于 这里的统计包括查询的SST个数，不管本次查询是否查到真实value
  DB_SEEK_2_TIMES,
  DB_SEEK_3_TIMES,
  DB_SEEK_4_TIMES,
  DB_SEEK_5_TIMES,
