...
inline void BlockFetcher::ProcessTrailerIfPresent() {
  if (footer_.GetBlockTrailerSize() > 0) {
    ...
    // RocksDB用最后一个bit表示compression_type 我们没有这个type...直接默认NoCompressiono
    compression_type_ =
...
IOStatus BlockFetcher::ReadBlockContents() {
  ...
  if (TryGetFromPrefetchBuffer()) {
    ...
    // Actual file read
    if (io_status_.ok()) {
      StopWatch sw(ioptions_.clock, ioptions_.stats, DB_DATA_DISK_GET);
      if (file_->use_direct_io()) {
        PERF_TIMER_GUARD(block_read_time);
        // std::cout << "direct file name" << file_->file_name() << std::endl;
        ...
        // std::cout << "file name" << file_->file_name() << std::endl;
