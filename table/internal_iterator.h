...
class InternalIteratorBase : public Cleanable {
 public:
  ...
  // 仅当当前是vertical compaction，且本key的初始kType为kTypeMerge时，vertical_merge_type == true,value()应该返回real value
  // 仅当当前是gc compaction时，tables不为空，value()是否返回real value取决于当前查询的value是否在tables列表中的table中
  virtual TValue value(bool vertical_merge_type, const std::set<uint32_t>& tables = std::set<uint32_t>()) const = 0;
  // 本函数返回本iterator是来自第几个level的(如果是lsm-tree中的iter)
  virtual int level() { return -1; };

  // 标识当前的value()是否为真实value值
  virtual bool real_value() const { return false; };

  // 标识当前iterator是由哪个Index/Table指向的
