...
class IteratorWrapperBase {
 public:
  ...
  TValue value(bool vertical_merge_type, const std::set<uint32_t>& tables = std::set<uint32_t>()) const {
    assert(Valid());
    return iter_->value(vertical_merge_type, tables);
  }
  uint64_t file_number() const {
    assert(Valid());
    return iter_->file_number();
  }
  bool real_value() const {
    assert(Valid());
    return iter_->real_value();
  }
