...
class TableBuilder {
 public:
  ...
  // 仅在NvmPartitionBuilder中使用
  virtual void Add(const Slice& key, const Slice& value, bool real_value) = 0;

  // 仅在NvmTable中有用，添加一个value并返回添加之前的offset。
  virtual uint64_t Add2(const Slice& value) = 0;
  ...
  virtual uint64_t TableNumEntries() const {return 0; };

  virtual uint64_t MergeEntries() const{return 0;};
  ...
  virtual uint64_t TableFileSize() const = 0;
