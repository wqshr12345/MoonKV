...
private:
 // 如果CacheEntry的资源是BLockCache中的内容，那么该资源的生命周期由BlockCache管理，CacheEntry只需要释放这个句柄
 // 如果CacheEntry的资源是自己拥有(比如直接从文件中读取，但没有保存到Cache中的数据)，那么该资源的生命周期由CacheEntry管理，CacheEntry需要释放这个资源
