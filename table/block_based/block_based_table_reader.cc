...
Status BlockBasedTable::Get(const ReadOptions& read_options, const Slice& key,
  ...
  // 如果key比当前block的最低值还要小，那么直接break，说明key一定不存在于当前table中
  // 因为之前的block遍历过却没找到key，同时key比当前block小，那么之后的block也一定找不到key。
      ...
      Status tmp_status;
      // 下面这个迭代器的生成是核心中的核心。它里面具体经历了一个block的查询的完整过程：cache->磁盘
      // Question：目前来看，应该是根据BlockHandle中的offset和size，从磁盘中读取block。但是为什么这个Block不是block_size对齐的？？？
      // 另外，写入数据的时候，虽然是数据到了block_size就写到磁盘，但是中间有一个compress过程呀，这样写入也用空间浪费？
      // 还是说，根本不需要block对齐？只要是4kb读、写就行？？？
...
Status BlockBasedTable::Get2(const ReadOptions& readOptions, const uint64_t& offset,
