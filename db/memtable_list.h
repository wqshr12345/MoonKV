...
class MemTableList {
 public:
  ...
  // func: 尝试将一个memtable的flush后的fmd添加到下一个version中 同时将该memtable从imm列表中删除
  // 为什么是尝试？因为version的fmd的添加一定要按照顺序 如果一个较新的memtable先flush完而较老的memtable尚未flush 那么前者不能添加fmd 必须等后者完成后一起进行
