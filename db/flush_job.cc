...
void FlushJob::PickMemTable() {
  ...
  // 把level0的文件存储到path:level0，注意[0]的位置是原文件夹路径，[1]是存的nvm文件夹路径；现在所有的table文件都统一存在原路径下
  meta_.fd = FileDescriptor(versions_->NewFileNumber(), 1, 0);
  table_meta_.fd = FileDescriptor(versions_->NewFileNumber(), 0, 0);
...
Status FlushJob::Run(LogsWithPrepTracker* prep_tracker, FileMetaData* file_meta,
  ...
  std::cout<<"Version Before Flush"<<std::endl;
  std::cout<<"LSM-Tree "<<std::endl;
    for(int i = 0;i<7;i++){
      uint64_t file_size = 0;
      std::cout<<"level "<<i<<": ";
      std::cout<<"total "<<cfd_->current()->storage_info()->LevelFiles(i).size()<<" ";
      for(auto& f:cfd_->current()->storage_info()->LevelFiles(i)){
        std::string status = f->being_compacted?"1":"0";
        std::cout<<f->fd.file_size <<" [start:"<<f->smallest.user_key().ToString()<<", end:"<<f->largest.user_key().ToString()<<", "<<status<<"] "<<f->fd.GetNumber()<<" | "; 
        // std::cout<<" |file "<<f->fd.GetNumber()<<" file size "<<f->fd.file_size<<" status "<<f->being_compacted<<" path_id "<<f->fd.GetPathId();; //<<" compacting "<<f->being_compacted<<" ";
        // file_size += f->fd.file_size;
      }
      std::cout<<std::endl;
      // std::cout<<" level size "<<file_size<<" level score "<<cfd_->current()->storage_info()->CompactionScore(i)<<std::endl;
    }
  //   std::cout<<"table files "<<std::endl;
  //   for(auto& f:cfd_->current()->storage_info()->AllTableFiles()){
  //     std::cout << "file "<<f.first<<" size "<<f.second->fd.GetFileSize()<<" pathid "<<f.second->fd.GetPathId()<<std::endl;
  //   }
  //   std::cout<<"index to tables"<<std::endl;
  //   for(int i = 0;i<4;i++){
  //     for(auto& f:cfd_->current()->storage_info()->LevelFiles(i)){
  //       std::cout<<"index "<<f->fd.GetNumber();
  //       for(auto m:f->fd.sub_number_to_reference_key){
  //         std::cout<<" table "<<m.first<<" ref "<<m.second;
  //       }
  //       std::cout<<std::endl;
  //     }
  //   }
  ...
  if (!s.ok()) {
    ...
    // WQTODOIMP 不确定下面要传入meta_的number还是table_meta_的number... 应该问题不大，都可以。这里用table_meta吧
    s = cfd_->imm()->TryInstallMemtableFlushResults(
        cfd_, mutable_cf_options_, mems_, prep_tracker, versions_, db_mutex_,
        table_meta_.fd.GetNumber(), &job_context_->memtables_to_free, db_directory_,
  ...
  std::cout<<"Version After Flush"<<std::endl;
  std::cout<<"LSM-Tree "<<std::endl;
    for(int i = 0;i<7;i++){
      uint64_t file_size = 0;
      std::cout<<"level "<<i<<": ";
    ...
    // 写guard...
    // 
    std::cout << "Estimate Search" << std::endl;
    for (int i = 0;i < 7;i ++) {
      std::cout << "level " << i << ": " << std::endl;
      for (auto& f : cfd_->current()->storage_info()->LevelFiles(i)) {
        std::cout << "file " << f->fd.GetNumber() << ": " << std::endl;
        for (auto position_key_list : f->children_ranks_) {
          std::cout << "{";
          for(int j = 0; j < position_key_list.Size(); j ++) {
            std::cout<< "[key: " << position_key_list[j].PositionKey << " " <<position_key_list[j].LowerLevelPos << " " << position_key_list[j].HigherLevelPos << "] ";
          }
          std::cout << "}" << std::endl;
          std::cout << std::endl;
        }
  ...
  if (s.ok() && file_meta != nullptr) {
    // 根据db_impl_compaction_flush中的相关逻辑来看，这里用table_meta_更恰当一点
    *file_meta = table_meta_;
  }
  RecordFlushIOStats();
...
Status FlushJob::WriteLevel0Table() {
...

  {
...

    {
      ...
      table_meta_.oldest_ancester_time = oldest_ancester_time;
      meta_.file_creation_time = current_time;
      table_meta_.file_creation_time = current_time;
      ...
      // TableBuilder构造的table，所以number使用的table_meta_
      ...
      s = BuildTable(
          ...
          BlobFileCreationReason::kFlush, seqno_to_time_mapping_, &table_meta_,event_logger_,
          job_context_->job_id, io_priority, &table_properties_, write_hint,
          full_history_ts_low, blob_callback_, &num_input_entries,
          &memtable_payload_bytes, &memtable_garbage_bytes,base_);
  ...
  if (s.ok() && has_output) {
    ...
    // that key range.
    // Add file to L0
    // 这里用meta_或者table_meta的seq、time、checksum等其实没有区别，它们都是一样的
    edit_->AddFile(0 /* level */, meta_.fd.GetNumber(),meta_.fd.GetPathId(),meta_.children_ranks_, meta_.total_entries_,meta_.merge_entries_, meta_.fd.GetSubNumberToReferencekey(),
    ...
    edit_->AddTableFile(table_meta_.fd.GetNumber(),table_meta_.fd.GetPathId(),table_meta_.total_entries_, 
                   table_meta_.fd.GetFileSize(), table_meta_.smallest, table_meta_.largest,
                   table_meta_.fd.smallest_seqno, table_meta_.fd.largest_seqno,
                   table_meta_.marked_for_compaction, table_meta_.temperature,
                   table_meta_.oldest_blob_file_number, table_meta_.oldest_ancester_time,
                   table_meta_.file_creation_time, table_meta_.file_checksum,
                   table_meta_.file_checksum_func_name, table_meta_.min_timestamp,
                   table_meta_.max_timestamp, table_meta_.unique_id);
...
// WQTODO 未来再来实现Flush线程中的Intralevel0Compaction。
// 核心问题是我们现在构造一个NvmPartition是通过一个大的CompactionIter完成，而不是让NvmPartition接受几个MemTable的迭代器。
// 我们期望使用RocksDB原生的MergeIter，而不是自己的内部的。这就要求我需要先把Compaction那里关于这的部分搞完再说。
Status FlushJob::IntraLevel0Compaction(){
  db_mutex_->AssertHeld();
  return Status::OK();
}
...
std::unique_ptr<FlushJobInfo> FlushJob::GetFlushJobInfo() const {
  ...
  // table file的number应该使用table_meta
  const uint64_t file_number = table_meta_.fd.GetNumber();
  info->file_path =
