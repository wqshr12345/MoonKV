...
class VersionStorageInfo {
 public:
  ...
  void ReserveGuard(int level, size_t size) { guard_[level].reserve(size); }

  // 注意，这里增加的两个函数对table_files_的操作类似RocksDB对files_的操作，重新生成一个新的map。
  // 根据AddFile接口中的结果，重新增加table_files_。
  
  // // func: 更改上一version中某个table_file的引用计数(这里特指index file对它的引用计数)
  // void ChangeTableReference(int changes,uint32_t file_number);

  // func: 本函数用于在一次version变更中增加一个table
  void AddTableFile(FileMetaData* f);

  // func: 本函数用于在一次version变更中增加一个guard
  void AddGuard(int level,std::string guard); 
  
  void AddFile(int level, FileMetaData* f);

  // 为什么这里只有相关元素的Add函数没有Delete函数？因为一个新VersionStorage的构造是从0开始add的，而不是copy了上一个VersionStorageInfo然后add&&delete.
  ...
  // func: 计算当前version中的每个level的compaction score，以及计算当前应当回收的table文件。
  // 注意，本函数在AppendVersion中被调用，也就是每个version被添加的时候就计算分数了。
  void ComputeCompactionScore(const ImmutableOptions& immutable_options,
                              const MutableCFOptions& mutable_cf_options);

  // func: 计算当前version中各个table的GC score,同时选择需要被value compaction的index(此处的GC score包括因为优化范围查询和空间放大而得到的score)
  void ComputeGCScore(const ImmutableOptions& immutable_options);

  //func：计算当前version中的各个index的VC score，同时将score按照大小排序...
  void ComputeVCscore(const ImmutableOptions& immutable_options);
  ...
  const std::vector<std::pair<uint32_t,std::pair<double,std::set<uint32_t>>>> GetIndexToScoreTablesSort() const {
    return index_to_score_tables_sort_;
  }

  const std::vector<std::vector<std::pair<uint64_t,double>>>& GetIndexToVcScore() const {
    return index_to_vc_score_;
  } 
  ...
  const std::unordered_map<uint32_t,FileMetaData*>& AllTableFiles() const {
    return table_files_;
  }

  const FileMetaData* GetTableFile(uint32_t file_number) const{
    std::unordered_map<uint32_t,FileMetaData*>::const_iterator it = table_files_.find(file_number);
  ...
  const std::vector<std::string> LevelGuard(int level) const {
    return guard_[level];
  }

  // 返回从从第1层到第level层的所有guard(这才是level层实际用到的guard)
  std::vector<std::string> UpGuard(int level) {
    std::vector<std::string> up_guard_;
    for(int i = 1; i <= level; i++){
      for(auto& guard : guard_[i]){
        up_guard_.push_back(guard);
      }
    }
    std::sort(up_guard_.begin(), up_guard_.end());
    return up_guard_;
  }
  ...
  // func: 根据table_file_number查找table_file的FileMetaData
  FileMetaData* GetTableFileMetaDataByNumber(uint64_t table_file_number) const{
    if(table_files_.find(table_file_number) != table_files_.end()){
      return table_files_.at(table_file_number);
    }
    return nullptr;
  }
 ...
 private:
  ...
  // 存储每个version中的table_files的file_number与FileMetaData的对应
  std::unordered_map<uint32_t,FileMetaData*> table_files_;

  // 存储当前的guard分区 为什么不用set保证有序？因为有序是在VersionBuilder的SaveGuardsTo()中通过comparator保证的
  std::vector<std::string>* guard_;
  ...
  std::map<uint32_t,std::multimap<uint32_t,uint32_t>> index_to_refs_tables_for_rangegc; // 存储因为范围查询触发gc的index 和 它引用的table及ref 的对应
  std::vector<std::pair<uint32_t,std::pair<double,std::set<uint32_t>>>> index_to_score_tables_for_rangegc; //上面这个map按照m.second的容量大小进行排序后的结果，同时计算得到了score值

  std::map<uint32_t,uint32_t> gc_index_to_refs_for_rangegc; // 存储因为范围查询触发的gc的index的reference_keys总数
  std::map<uint32_t,std::pair<double,std::set<uint32_t>>> index_to_score_tables_for_spacegc; // 存储因为空间放大触发gc的index 和 它的score及引用的tables 的对应
  // std::vector<std::pair<uint32_t,std::pair<double,std::vector<uint32_t>>>> index_to_score_tables_for_spacegc_sort; // 上面这个map按照score大小进行排序

  std::vector<std::pair<uint32_t,std::pair<double,std::set<uint32_t>>>> index_to_score_tables_sort_; // 将上面两个sort的内容合并(归并排序)
  std::map<uint32_t,std::pair<double,std::set<uint32_t>>> index_to_score_tables_;
  
  
  std::vector<std::vector<std::pair<uint64_t,double>>> index_to_vc_score_; // indexs_to_vc_score_[i][j].first表示第i level的merge_entries_占比第j多的index，其file number；indexs_to_vc_score_[i][j].second代表具体merge_entries_占比
...
class Version {
 public:
  ...
  void Get(const ReadOptions&, const LookupKey& key, PinnableSlice* value,
           ...
           bool* is_blob = nullptr, bool do_merge = true, int* merge_keys = nullptr, int* merge_levels = nullptr, int* merge_times = nullptr, int* merge_keys_nol0 = nullptr);
  ...
  // *live_blob_files.
  void AddLiveFiles(std::vector<uint64_t>* live_index_files,std::vector<uint64_t>* live_table_files,
...
class VersionSet {
 public:
  ...
  // func: 获取一次sub compaction的所涉及到所有文件的迭代器(实际上不只这个sub compaction，整个大compaction的几乎所有文件都会涉及到)
  ...
  void GetObsoleteFiles(std::vector<ObsoleteFileInfo>* files,std::vector<ObsoleteFileInfo>* table_files,
  ...
  EstimateSeekTable* estimate_search_table() {
    return &estimate_search_table_;
  }
 ...
 protected:
  ...
  std::vector<ObsoleteFileInfo> obsolete_table_files_;
  ...
  //并发安全的hash table 用于在EstimateSearch开启后 动态增减变化
  // folly::ConcurrentHashMap<std::pair<uint64_t, uint64_t>, PositionKeyList, pair_hash> estimate_search_table_;
  EstimateSeekTable estimate_search_table_;
 private:
