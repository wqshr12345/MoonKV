...
class VersionBuilder::Rep {
  ...
  class ByGuard {
   public:
    explicit ByGuard(const InternalKeyComparator* cmp) : cmp_(cmp) {}

    bool operator()(const std::string lguard, const std::string rguard) const {
      assert(cmp_);

      // guard没有value type和seq等乱七八糟的东西，直接用user_comparator()比较就好
      const int r = cmp_->user_comparator()->Compare(lguard, rguard);
      if (r != 0) {
        return (r < 0);
      }
      // 理论上不会发生lguard == rguard的情况 我们会在MayBeAddGuard中有去重逻辑
      return true;
    }
  ...
  struct LevelState {
    ...
    std::unordered_set<std::string> deleted_guards;
    
    std::unordered_set<std::string> added_guards;
  };
  ...
  int num_levels_;
  LevelState* levels_;
  std::unordered_map<uint64_t, FileMetaData*> added_table_files_;
 ...
 public:
  Rep(const FileOptions& file_options, const ImmutableCFOptions* ioptions,
      ...
      : file_options_(file_options),
        ...
        guard_cmp_(base_vstorage_->InternalComparator()),
  ...
  Status ApplyFileDeletion(int level, uint64_t file_number) {
    ...
    if (add_it != add_files.end()) {
      // WQTODO 下面的UnrefFile是否有必要？只要在add_files中删掉了，VersionBuilder析构的时候自然会把这个ref--，然后变为0，释放掉？
  ...
  Status ApplyTableFileAddition(const FileMetaData& meta){
    const uint64_t file_number = meta.fd.GetNumber();

    FileMetaData* const f = new FileMetaData(meta);
    // 无需在此处设1，避免在VersionBuilder的析构函数中refs--
    //f->refs = 1;

    // 避免添加多个相同file number的NvmTableFile
    assert(added_table_files_.find(file_number) == added_table_files_.end());
    added_table_files_.emplace(file_number, f);

    return Status::OK();
  }

  Status ApplyGuardDeletion(int level,std::string guard){
    assert(level != VersionStorageInfo::FileLocation::Invalid().GetLevel());

    auto& level_state = levels_[level];

    auto& del_guards = level_state.deleted_guards;
    assert(del_guards.find(guard) == del_guards.end());
    del_guards.emplace(guard);

    return Status::OK();
  }
  Status ApplyGuardAddition(int level,std::string guard){

    assert(level != VersionStorageInfo::FileLocation::Invalid().GetLevel());

    auto& level_state = levels_[level];

    // 如果同一个string既被删除又被添加，那么就取消删除它 WQTODO 这个逻辑是否适用guard呢
    auto& del_guards = level_state.deleted_guards;
    auto del_it = del_guards.find(guard);
    if (del_it != del_guards.end()) {
      del_guards.erase(del_it);
    }

    auto& add_guards = level_state.added_guards;
    assert(add_guards.find(guard) == add_guards.end());
    add_guards.emplace(guard);
  ...
  Status Apply(const VersionEdit* edit) {
    ...
    // 增加新的 table files(此处table file特指磁盘上的NvmTable)
    for (const auto& new_table_file_meta : edit->GetNewTableFiles()){
      const Status s = ApplyTableFileAddition(new_table_file_meta);
    }

    // 删除guards
    for (const auto& deleted_guard : edit->GetDeletedGuards()) {
      const int level = deleted_guard.first;
      const std::string guard = deleted_guard.second;

      const Status s = ApplyGuardDeletion(level, guard);
      if (!s.ok()) {
        return s;
      }
    }
    ...
    for (const auto& new_guard : edit->GetNewGuards()) {
      ...
      const std::string guard = new_guard.second;

      const Status s = ApplyGuardAddition(level, guard);
      if (!s.ok()) {
        return s;
      }
    }
  ...
  // func：本函数用来向新version中添加file_数组中需要的FileMetaData
  // 此处传入的FileMetaData已经经过了上一层的筛选 保留了上一version的所有file 增加了本次flush/compaction中新增的file
  // 如果传入了删除的file，会在下面del_it!=del_files.end()的情况下删除
  void MaybeAddFile(VersionStorageInfo* vstorage, int level,
    ...
    if (del_it != del_files.end()) {
      ...
      // 实际上当前的guard要么来自base_file要么来自add_file。下面的优化仅仅是为了当base_file和add_files出现相同的file_number时，选择base_file中。
  ...
  // func: 本函数用来向新version中添加table_files_的map中需要的FileMetaData
  void AddTableFile(VersionStorageInfo* vstorage,FileMetaData* f) const {
    vstorage->AddTableFile(f);
  }
  // func: 本函数用来向新version中添加guard_中需要的guard
  void MaybeAddGuard(VersionStorageInfo* vstorage,int level,std::string guard) const {

    const auto& level_state = levels_[level];

    const auto& del_guards = level_state.deleted_guards;
    const auto del_it = del_guards.find(guard);

    // 只有当前的guard不在del列表中，才会添加到下一version
    if (del_it == del_guards.end()) {
        vstorage->AddGuard(level, guard);
    }  
  }

  // func: 本函数用来生成新version中的所有table file
  void SaveTableFilesTo(VersionStorageInfo* vstorage) const {
    // 获取上一version的所有table file的集合
    const auto& base_table_files = base_vstorage_->AllTableFiles();
    // 本version新增的所有NvmTableFile集合在added_table_files_中

    // 合并base和add中的所有<file_number,FileMetaData>
    // WQTODO 如果added_table_files_为空，这里可以优化一下，直接用base_table_files而不是用一个新的map
    std::unordered_map<uint32_t,FileMetaData*> total_files;
    total_files.insert(added_table_files_.begin(),added_table_files_.end());
    total_files.insert(base_table_files.begin(),base_table_files.end());


    // 由于在此之前已经经过了SaveSSTFilesTo()函数，所以vstorage中存储的files_就是新version中的所有index file

    // 可以简单用VersionBuilder中的delete和add的信息来实现(但是注意，这里的Add是MayBeAdd，也就是说add中的file并不一定真正add到了下一版本，所以这么实现可能存在bug)
    // 可以尝试在MayBeAdd中打断点，看是否走了没有AddFile的逻辑。如果有，那么不可以用这样的策略，还是老老实实用total_files实现。
    // 必须遍历两次 先将所有add的引用计数增加 再把所有del的引用计数删除 这样才能避免误删
    if(total_files.size()>0){
      for (int level = 0; level < num_levels_; level++) {
        // 为add file中涉及的所有sub_numbers增加引用计数
        for (const auto& file_meta_pair : levels_[level].added_files) {
          FileMetaData* add_file = file_meta_pair.second;
          uint64_t index_file_number = add_file->fd.GetNumber();
          for(const auto number_to_reference : add_file->fd.GetSubNumberToReferencekey()){
            uint32_t table_number = number_to_reference.first, reference = number_to_reference.second;
            // 根据AddFile 把其对应的table的引用计数增加 同时把其引用key的数量和对应的father_number_to_reference_key增加。
            FileMetaData* sub_table = total_files.find(table_number)->second;
            sub_table->refs++;
            sub_table->reference_entries_ += reference;
            sub_table->fd.father_number_to_reference_key[index_file_number] += reference;
          }
        }
      }
      
      for (int level = 0; level < num_levels_; level++) {
        // 为delete file中涉及的所有sub_numbers减少引用计数
        for (uint64_t file_number : levels_[level].deleted_files) {
          FileMetaData* del_file = base_vstorage_->GetFileMetaDataByNumber(file_number);
          uint64_t index_file_number = del_file->fd.GetNumber();
          for(const auto number_to_reference : del_file->fd.GetSubNumberToReferencekey()){
            uint32_t table_number = number_to_reference.first, reference = number_to_reference.second;
            FileMetaData* sub_table = total_files.find(table_number)->second;
            sub_table->refs--;
            sub_table->reference_entries_ -= reference;
            sub_table->fd.father_number_to_reference_key[index_file_number] -= reference;
            // 如果某个index不再引用某个table，那么就将其从father_number_to_reference_key中删除
            if(sub_table->fd.father_number_to_reference_key[index_file_number] == 0){
              sub_table->fd.father_number_to_reference_key.erase(index_file_number);
            }
            
            // 理论上这里不会出现total_files.find(table_number)出现空指针的情况。因为如果total_files删掉了table_number,后续应该不会有del_file的sub_number再指向这个table_number
            if(sub_table->refs == 0){
              total_files.erase(table_number);
            }
          }
        }
      }
    }
    // 当前total_files中存储的就是下一version仍然存在的NvmTableFile
    for(auto it:total_files){
      AddTableFile(vstorage,it.second);
    }
    // // 合并base和add中的所有file_number
    // std::unordered_set<uint32_t> total_file_numbers;
    // for(auto it = base_table_files.begin();it!=base_table_files.end();it++){
    //   total_file_numbers.insert(it->first);
    // }
    // for(auto it = added_table_files_.begin();it!=added_table_files_.end();it++){
    //   total_file_numbers.insert(it->first);
    // }
    // // 这里考虑一下，可以不用map存引用计数，而是在下面，每找到一次，就++fmd的引用计数。前提是先清空引用计数。
    // std::unordered_map<FileMetaData*,uint32_t> temp_table_files; // 存储新version下引用计数和FileMetaData的对应
    // for(int i = 0;i<vstorage->num_levels();i++){
    //   auto& files = vstorage->LevelFiles(i);
    //   for(auto& file_meta : files){
    //     std::vector<uint32_t> sub_numbers = file_meta->fd.GetSubNumbers();
    //     for(auto number : sub_numbers){
    //       FileMetaData* fmd = total_files.find(number)->second; // 用find为了避免number不存在的情况，提早暴露错误
    //       temp_table_files[fmd]++;

    //       total_file_numbers.erase(number);
    //     }
    //   }
    // }
    // for(auto it:temp_table_files){
    //   it.first->fd.SetIndexRefs(it.second);
    //   AddTableFile(vstorage,it.first);
    // }
    // for(auto file_number:total_file_numbers){
    //   base_vstorage_->ObsoleteTableFiles().push_back(base_table_files.find(file_number)->second);
    // }
  }
  template <typename Cmp>
  void SaveGuardsTo(VersionStorageInfo* vstorage,int level, Cmp cmp) const{
    // 获取上一version的所有guard file的集合
    const auto& base_guard = base_vstorage_->LevelGuard(level);
    // 本version新增的所有NvmTableFile集合在added_table_files_中
    const auto& unordered_added_guards = levels_[level].added_guards;
    vstorage->ReserveGuard(level, base_guard.size() + unordered_added_guards.size());

    // Sort added files for the level.
    std::vector<std::string> added_guards(unordered_added_guards.begin(),unordered_added_guards.end());
    std::sort(added_guards.begin(), added_guards.end(), cmp);

    auto base_iter = base_guard.begin();
    auto base_end = base_guard.end();
    auto added_iter = added_guards.begin();
    auto added_end = added_guards.end();
    while (added_iter != added_end || base_iter != base_end) {
      if (base_iter == base_end ||
          (added_iter != added_end && cmp(*added_iter, *base_iter))) {
        MaybeAddGuard(vstorage, level, *added_iter++);
      } else {
        MaybeAddGuard(vstorage, level, *base_iter++);
      }
    }
  }
  ...
  void SaveGuardsTo(VersionStorageInfo* vstorage) const {
    ...
    // guard从level1开始才有效
    for (int level = 1; level < num_levels_; ++level) {
      SaveGuardsTo(vstorage,level,guard_cmp_);
    }
  }

  // func：在经历一次version的变化(包括各个table的引用计数变化 各个index和table的numberToreference的变化)之后，验证这些引用计数是对应的
  void CheckIndexAndTableConsistency(VersionStorageInfo* vstorage) const {
    std::map<std::pair<uint32_t,uint32_t>,uint32_t> index_number_table_number_to_reference1,index_number_table_number_to_reference2;
    std::map<uint32_t,uint32_t> table_number_to_reference1,table_number_to_reference2;
    // 根据index file的sub信息构造
    for(int i = 0;i<vstorage->num_levels();i++){
      for(auto& f: vstorage->LevelFiles(i)){
        const uint32_t index_number = f->fd.GetNumber();
        const std::map<uint32_t,uint32_t> sub_numbers_to_reference_key = f->fd.GetSubNumberToReferencekey();
        for(auto m : sub_numbers_to_reference_key){
          index_number_table_number_to_reference1[{index_number,m.first}] += m.second;
        }
      }
    }
    // 根据table file的father信息构造
    for(auto& f: vstorage->AllTableFiles()){
      const uint32_t table_number = f.first;
      const FileMetaData* table_meta = f.second;
      assert(table_number == table_meta->fd.GetNumber());
      const std::map<uint32_t,uint32_t> father_number_to_reference_key = table_meta->fd.GetFatherNumberToReferencekey();
      table_number_to_reference1[table_number] += table_meta->reference_entries_;
      for(auto m : father_number_to_reference_key){
        index_number_table_number_to_reference2[{m.first,table_number}] += m.second;
      }
    }
    assert(index_number_table_number_to_reference1.size() == index_number_table_number_to_reference2.size());
    std::map<std::pair<uint32_t,uint32_t>,uint32_t>::iterator it1 = index_number_table_number_to_reference1.begin();
    std::map<std::pair<uint32_t,uint32_t>,uint32_t>::iterator it2 = index_number_table_number_to_reference2.begin();
    while(it1!=index_number_table_number_to_reference1.end() && it2!=index_number_table_number_to_reference2.end()){
      assert(it1->first.first == it2->first.first && it1->first.second == it2->first.second && it1->second == it2->second);
      table_number_to_reference2[it1->first.second] += it1->second;
      it1++;
      it2++;
    }
    // std::cout<<"table_number_to_reference1"<<std::endl;
    // for(auto i :table_number_to_reference1) {
    //   std::cout<<"table number "<<i.first<<" ref "<<i.second<<std::endl;
    // }
    // std::cout<<"table_number_to_reference2"<<std::endl;
    // for(auto i :table_number_to_reference2) {
    //   std::cout<<"table number "<<i.first<<" ref "<<i.second<<std::endl;
    // }
    assert(table_number_to_reference1.size() == table_number_to_reference2.size());
    std::map<uint32_t,uint32_t>::iterator it3 = table_number_to_reference1.begin();
    std::map<uint32_t,uint32_t>::iterator it4 = table_number_to_reference2.begin();
    while(it3!=table_number_to_reference1.end() && it4!=table_number_to_reference2.end()){
      assert(it3->first == it4->first && it3->second == it4->second);
      it3++;
      it4++;
    }
  }
  ...
  void SaveSSTFilesTo(VersionStorageInfo* vstorage) const {
    ...
    // 针对L0是不同的添加策略。因为L0的文件互相重叠，这里的顺序应该按照memtable的顺序来添加，即按照不同sst的最大的seqno排序添加。
  ...
  Status SaveTo(VersionStorageInfo* vstorage) const {
    ...
    // 虽然名字叫SaveSSTFles 但实际上Save的是Nvm中的Index File
    SaveSSTFilesTo(vstorage);

    // Save SSD上的Table File对应的fmd
    SaveTableFilesTo(vstorage);

    // 保存本次version变化中增减的guard
    SaveGuardsTo(vstorage);

    // 检查本次version变化前后 table file的各个引用计数是否出现问题...
    CheckIndexAndTableConsistency(vstorage);
