...
struct JobContext {
  inline bool HaveSomethingToDelete() const {
    return !(full_scan_candidate_files.empty() && index_delete_files.empty() &&
             blob_delete_files.empty() && log_delete_files.empty() &&
             manifest_delete_files.empty() && table_delete_files.empty());
  }
  ...
  std::vector<uint64_t> index_live;

  std::vector<uint64_t> table_live;

  // the list of index files that we need to delete
  std::vector<ObsoleteFileInfo> index_delete_files;

  // 需要删除的table files的列表
  std::vector<ObsoleteFileInfo> table_delete_files;
