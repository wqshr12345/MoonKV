...
class FlushJob {
 ...
 private:
  ...
  // 为什么IntraLevel0Compaction()要在Flush线程做？是为了让它的优先级更高一些，Compaction线程的优先级相比Flush要低一些。
  Status IntraLevel0Compaction();
  ...
  FileMetaData meta_;       // 此meta用于存储生成的NvmBtree/NvmPartition信息
  FileMetaData table_meta_; // 此meta用于存储生成的NvmTable信息
