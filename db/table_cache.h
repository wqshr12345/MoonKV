...
class TableCache {
 public:
  ...
  // func: 本函数用来根据file_seq得到一个具体的TableReader，同时将这个TableReader插入cache。会在两个地方被调用
  // 1.Version::Get()中，利用FindTable()得到一个NvmPartition
  // 2.NvmPartition::Get()中，利用FindTable()得到一个NvmTable
  // 针对两处的不同，增加一个bool变量，用于生成不同的TableReader
  // 在更外层，代码会从cache中得到这个具体的TableReader，然后存入fmd的fd的table_reader中
  ...
  Status FindTable(
      ...
      Temperature file_temperature = Temperature::kUnknown,bool is_table = false);
