...
class SubcompactionState {
 public:
  ...
  void AddOutputsEdit(VersionEdit* out_edit) const {
    ...
    for(const auto& file : compaction_outputs_.table_outputs_){
      // IMP 加这一行逻辑是为了避免出现 某个guard内的table并没有添加任何数据 却被放到了meta里面 这会在后面一致性检验中出错
      if(file.meta.total_entries_ != 0)
        out_edit->AddTableFile(file.meta);
    }
    for(auto g:compaction_outputs_.guards_){
      for(auto guard:g.second)
        out_edit->AddGuard(g.first,guard);
    }
  }
