...
Compaction::Compaction(
    ...
    bool is_gc_compaction,
    bool is_vertical_compaction,
    int vertical_start_level)
    : input_vstorage_(vstorage),
      ...
      is_gc_compaction_(is_gc_compaction),
      is_vertical_compaction_(is_vertical_compaction),    
      vertical_start_level_(vertical_start_level) {
  if(is_vertical_compaction_) {
    // 在vertical compaction情况下，必须保证inputs_[0]中的所有文件都来自start level,我们后续的MarkFilesBeingCompacted依赖于这一事实
    assert(inputs_[0].level == vertical_start_level_);
    // WQTODO 这里的实现其实不太优雅... 我们这样的逻辑是建立在vertical_compaction一定能涉及到bottom level的情况下
...
void Compaction::AddInputDeletions(VersionEdit* out_edit) {
  for (size_t which = 0; which < num_input_levels(); which++) {
    for (size_t i = 0; i < inputs_[which].size(); i++) {
      // 针对vertical compaction 非start_level的文件不应该被删除
      if(is_vertical_compaction_ && level(which) != vertical_start_level_) 
          continue;
...
void Compaction::MarkFilesBeingCompacted(bool mark_as_compacted) {
  for (size_t i = 0; i < num_input_levels(); i++) {
    for (size_t j = 0; j < inputs_[i].size(); j++) {
        // 针对vertical compaction的文件，如果是非start level，那么不会将其置为being_compacted
      if(is_vertical_compaction_ && i!=0) {
        continue;
      }
...
bool Compaction::ShouldFormSubcompactions() const {
  ...
  if (cfd_->ioptions()->compaction_style == kCompactionStyleLevel) {
    // 为什么只在level0往后的地方触发sub compaction? 仅仅为了更快的l0->base level的压缩吗？
    // 我们这里应该修改这个错误...
    return output_level_ > 0;
