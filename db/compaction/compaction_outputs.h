...
class CompactionOutputs {
 public:
  ...
  void AddTableOutput(FileMetaData&& meta, const InternalKeyComparator& icmp,
                 bool enable_order_check, bool enable_hash,
                 bool finished = false, uint64_t precalculated_hash = 0) {
    table_outputs_.emplace_back(std::move(meta), icmp, enable_order_check,
                          enable_hash, finished, precalculated_hash);
  }
  ...
  // func:具体执行builder的finish的逻辑
  Status Finish(const Status& intput_status,
                const SeqnoToTimeMapping& seqno_time_mapping, bool gc_compaction = false, bool vertical_compaction = false);
  ...
  FileMetaData* GetTableMetaData() { return &current_table_output().meta; }
 ...
 private:
  ...
  // func: 添加每一个具体的key-value的逻辑
  Status AddToOutput(const CompactionIterator& c_iter,
                     const CompactionFileOpenFunc& open_file_func,
                     const CompactionFileCloseFunc& close_file_func,bool gc_compaction = false, bool vertical_compaction = false, bool first_pick_guard = false);
  ...
  Output& current_table_output() {
    assert(!table_outputs_.empty());
    return table_outputs_.back();
  }

  std::map<uint32_t,std::vector<std::string>>& current_guard(){
    return guards_;
  }
  ...
  std::vector<Output> table_outputs_;

  std::map<uint32_t,std::vector<std::string>> guards_;
