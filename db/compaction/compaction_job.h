...
class CompactionJob {
 public:
  ...
  // func: 提前为本次compaction准备好每个subcompaction的边界
  void Prepare();
 ...
 private:
  ...
  //func: 为sub compaction生成boundaries
  ...
  // func: 结束一次sub compaction，执行NvmPartitionBuilder的finish逻辑等。
  Status FinishCompactionOutputFile(const Status& input_status,
                                    SubcompactionState* sub_compact,
                                    CompactionOutputs& outputs,
                                    const Slice& next_table_min_key);
  Status InstallCompactionResults(const MutableCFOptions& mutable_cf_options);
  // func: 构造一次sub compaction的index file和table file的meta，打开NvmPartitionBuilder等逻辑
...
public:
  ...
  std::atomic<int>* first_pick_guard_;
private:
  ...
  virtual std::string GetIndexFileName(uint64_t file_number);
  ...
  // 针对file_meta指向的文件，重构其在当前level+1的所有children_ranks
  void UpdateChildrenRanksForLevel(int level, FileMetaData& file_meta, InternalIterator* father_iter);
};
