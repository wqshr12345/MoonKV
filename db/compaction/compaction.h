...
class Compaction {
 public:
  Compaction(VersionStorageInfo* input_version,
             ...
             bool is_gc_compaction = false,
             bool is_vertical_compaction = false,
             int vertical_start_level = -1);
  ...
  int vertical_start_level() const { return vertical_start_level_; }
  ...
  void set_is_gc_compaction(bool gc_compaction) {
    is_gc_compaction_ = gc_compaction;
  }

  void set_is_vertical_compaction(bool vertical_compaction){
    is_vertical_compaction_ = vertical_compaction;
  }

  // 将CompationPicker中的index_to_tables存储到Compaction中
  // 这里会将Vector形式的tables变为Set形式的tables
  void set_index_to_tables(const std::vector<std::pair<uint32_t,std::pair<double,std::set<uint32_t>>>>& index_to_tables){
    for(auto& m : index_to_tables){
      index_to_tables_.insert({m.first,m.second.second});
    }
  }

  void set_vertical_involve_levels(int vertical_involve_levels){
    vertical_involve_levels_ = vertical_involve_levels;
  }
  ...
  bool is_gc_compaction() const { return is_gc_compaction_; }

  bool is_vertical_compaction() const { return is_vertical_compaction_; }

  int vertical_involve_levels() const { return vertical_involve_levels_; };
  
  const std::map<uint32_t,std::set<uint32_t>> get_index_to_tables() const {
    return index_to_tables_;
  }
 ...
 private:
  ...
  // 如果为true，表明本次compaction是因为table的gc而触发的
  bool is_gc_compaction_ = false;

  // 如果为true，表明本次compaction需要进行一次针对Merge操作的vertical compaction
  bool is_vertical_compaction_ = false;

  int vertical_start_level_; // 表示当前vertical compaction的开始level 不再使用start_level作为vertical compaction的开始level参数

  int vertical_involve_levels_; // 表示当前vertical compaction涉及到的level

  std::map<uint32_t,std::set<uint32_t>> index_to_tables_;
