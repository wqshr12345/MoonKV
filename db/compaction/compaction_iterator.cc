...
CompactionIterator::CompactionIterator(
    ...
    const SequenceNumber penultimate_level_cutoff_seqno,bool flush, bool gc_compaction, std::map<uint32_t,std::set<uint32_t>> index_to_tables, bool vertical_compaction, int vc_start_level)
    : CompactionIterator(
          ...
          penultimate_level_cutoff_seqno,flush, gc_compaction, index_to_tables,vertical_compaction,vc_start_level) {}
...
CompactionIterator::CompactionIterator(
    ...
    : input_(input, cmp,
      ...
      index_to_tables_(index_to_tables),
      vertical_compaction_(vertical_compaction),
      vc_start_level_(vc_start_level){
...
void CompactionIterator::NextFromInput() {
  ...
  while (!valid_ && input_.Valid() && !IsPausingManualCompaction() &&
    ...
    if(gc_compaction_) {
      auto it = index_to_tables_.find(input_.file_number());
      if( it != index_to_tables_.end())
        tables = it->second;
    }
    // 这里一定是非vertical compaction下的Merge，因为vertical compaction下的Merge会在下面MergeUntil被处理。
    // 而如果这里的ktype为Merge是无所谓的。这里如果type为kTypeMerge，那么一定不是第一个，那么这里一定会被Put给覆盖掉。
    // 同时根据gc compaction选择对应的table
    if(flush_) {
      value_ = input_.value(true);
      real_value_ = input_.real_value();
    }else {
    value_ = input_.value(false, tables);
    real_value_ = input_.real_value();
    }
    now_level_ = input_.level();
    ...
    // 如果当前是vertical compaction,同时当前Index的level不是start_level,同时type不是kTypeMerge，那么我们不做任何变化，直接遍历到下一个元素...
    if( vertical_compaction_ && now_level_ != vc_start_level_ && ikey_.type != kTypeMerge ) {
      AdvanceInputIter();
    } else if (need_skip) {
    ...
    // 我们关注的最后一种情况——Merge.
    // 只有在当前为vertical_compaction的情况下才会走这里的逻辑，即合并多个merge的value
    } else if (ikey_.type == kTypeMerge && vertical_compaction_) {
      ...
      // 这是对Merge处理的逻辑，为了避免对已有逻辑的侵占，使用了新的merge_helper_,屎山就是这么来的...
      // 只有在第一次遇到Merge的时候会进入这个逻辑。在这个逻辑中会完全实现对Merge的处理；等这个方法出来之后，iter的指针已经指向下一个key了。
      ...
      if (!s.ok() && !s.IsMergeInProgress()) {
        ...
        // 这里是Merge操作合并之后的value
        key_ = merge_out_iter_.key();
        value_ = merge_out_iter_.value();
        // 参与Merge操作时，real_value都选择true
        real_value_ = true;
      ...
      // 3. Merge Type && not vertical_compaction
