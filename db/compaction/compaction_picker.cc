...
bool CompactionPicker::SetupOtherInputs(
  ...
  assert(!inputs->empty());
  // 注意此处有这个assert,说明直到SetupOtherInputsIfNeeded()调用SetupOtherInputs()时output_level_inputs是空的.
...
void CompactionPicker::RegisterCompaction(Compaction* c) {
  ...
  if (c->is_gc_compaction()) {
    gc_compactions_in_progress_.insert(c);
  }
...
void CompactionPicker::UnregisterCompaction(Compaction* c) {
  ...
  if (c->is_gc_compaction()) {
    gc_compactions_in_progress_.erase(c);
  }
}
