...
class SequenceIterWrapper : public InternalIterator {
 public:
  ...
  Slice value(bool vertical_merge_type, const std::set<uint32_t>& tables = std::set<uint32_t>()) const override { return inner_iter_->value(vertical_merge_type, tables); }

  bool real_value() const override{
    return inner_iter_->real_value();
  }

  uint64_t file_number() const override {
    return inner_iter_->file_number();
  }
  ...
  int level() override { return inner_iter_->level(); }
...
class CompactionIterator {
 public:
  ...
  CompactionIterator(
      ...
      const SequenceNumber penultimate_level_cutoff_seqno = kMaxSequenceNumber,bool flush = false, bool gc_compaction = false, std::map<uint32_t,std::set<uint32_t>> index_to_tables_ = std::map<uint32_t,std::set<uint32_t>>(), bool vertical_compaction = false, int vc_start_level = -1);
  ...
  CompactionIterator(
      ...
      const std::shared_ptr<Logger> info_log = nullptr,
      const std::string* full_history_ts_low = nullptr,
      const SequenceNumber penultimate_level_cutoff_seqno = kMaxSequenceNumber,bool flush = false, bool gc_compaction = false, std::map<uint32_t,std::set<uint32_t>> = std::map<uint32_t,std::set<uint32_t>>(), bool vertical_compaction = false, int vc_start_level = -1);
  ...
  const bool real_value() const { return real_value_; };
 ...
 private:
  ...
  bool gc_compaction_; // 标识本次Compaction是否为gc compactinon
  bool flush_; // 标识本次是否为flush的调用
  bool vertical_compaction_;// 标识本次Compaction是否为vertical compaction
  std::map<uint32_t,std::set<uint32_t>> index_to_tables_; //仅在gc_compaction_ == true 时有效
  ...
  bool real_value_; // 提供对real_value_的一层封装，避免后续取值出现valid()为false。
  ...
  // 标识当前遍历到的key所属Index的level
  int now_level_ = -1;

  // 标识vertical compaction的开始level，所有输出的Merge都会合并到这一level(仅在vertical_compaction_==true时有效)
  int vc_start_level_ = -1;
