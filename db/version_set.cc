...
Version::~Version() {
  ...
  for (int level = 0; level < storage_info_.num_levels_; level++) {
    for (size_t i = 0; i < storage_info_.files_[level].size(); i++) {
      ...
      f->refs--;
      if (f->refs <= 0) {
        vset_->estimate_search_table()->DestroyKeyPosList(f->fd.GetNumber());
        assert(cfd_ != nullptr);
        uint32_t path_id = f->fd.GetPathId();
        assert(path_id < cfd_->ioptions()->cf_paths.size());
        //files_中均为NvmBtree，应该使用nvm路径 即1号路径
  ...
  // NvmTable的回收逻辑
  // 注意，因为我们的ref的语义是当前有多少index指向table obsolete_table_files_里可能会存在一个table被删多次
  // 这本质上是因为一次compaction/flush之后，last version会被析构；同时，Compaction中，last version也会被析构...每次析构都会有这么一次调用...很烦~
  // 暂时在delete meta那里做特殊处理，如果还有问题，之后再说吧...
  #ifndef NDEBUG
  std::cout<<"Delete Table File :["<<std::endl;
  #endif
  for(auto it:storage_info_.table_files_){
    ...
    if(f->refs == 0 && !f->is_deleted){
        ...
        uint32_t path_id = f->fd.GetPathId();
        assert(path_id < cfd_->ioptions()->cf_paths.size());
        // 应该添加到obsolete_table_files_中
        std::cout<<"table "<<f->fd.GetNumber()<<" is deleted"<<std::endl;
        vset_->obsolete_table_files_.push_back(
            ObsoleteFileInfo(f, cfd_->ioptions()->cf_paths[path_id].path,
                             cfd_->GetFileMetadataCacheReservationManager()));
        f->is_deleted = true;
    }
...
class LevelIterator final : public InternalIterator {
 public:
  ...
  Slice value(bool vertical_merge_type, const std::set<uint32_t>& tables = std::set<uint32_t>()) const override {
    assert(Valid());
    return file_iter_.value(vertical_merge_type, tables);
  }

  bool real_value() const override {
    assert(Valid());
    return file_iter_.real_value();
  }
 ...
 private:
  ...
  InternalIterator* NewFileIterator() {
    ...
    // std::vector<uint32_t> selected_tables;
    // if(index_to_tables_.find(file_meta.fd.GetNumber())!=index_to_tables_.end()){
    //   selected_tables = index_to_tables_.at(file_meta.fd.GetNumber());
    // }
  ...
  IteratorWrapper file_iter_;  // May be nullptr // LevelIterator指向的某个SST的iterator
...
VersionStorageInfo::VersionStorageInfo(
  ...
  // 虽然l0没有guard，但还是构造num_levels_的长度，避免复杂逻辑中出错。
      guard_(new std::vector<std::string>[num_levels_]),
      // table_files_(new std::unordered_map<uint32_t,FileMetaData*>),
      ...
      index_to_vc_score_(num_levels_),
...
void Version::Get(const ReadOptions& read_options, const LookupKey& k,
                  ...
                  bool* is_blob, bool do_merge, int* merge_keys, int* merge_levels, int* merge_times, int* merge_keys_nol0) {
  ...
  GetContext get_context(
      ...
      tracing_get_id, &blob_fetcher, &vset_->estimate_search_table_);

  const Defer cleanup([merge_keys, merge_levels, merge_times, merge_keys_nol0, &get_context]() {
    if (merge_keys) {
      *merge_keys = int (get_context.get_context_stats_.num_seek_merge_key_times +
                    get_context.get_context_stats_.num_seek_full_key_times);
    }
    if(merge_levels) {
      *merge_levels = get_context.get_context_stats_.num_seek_levels;
    }
    if(merge_times) {
      *merge_times = get_context.get_context_stats_.num_seek_times;
    }
    if (merge_keys_nol0) {
      *merge_keys_nol0 = get_context.get_context_stats_.num_seek_key_times_nol0;
    }
  });
  get_context.get_context_stats_.num_search_keys = 1;
  // std::cout<<"search a new user_key"<<std::endl;
  ...
  #ifndef NDEBUG
  std::cout << "Start Read "<< k.user_key().ToString() << std::endl;
  #endif
  while (f != nullptr) {
    get_context.get_context_stats_.num_search_files += 1;
    ...
    // std::cout<<"search level "<<fp.GetCurrentLevel()<<"file "<<f->fd.GetNumber()<<std::endl;
    StopWatch* sw = new StopWatch(clock_, db_statistics_, DB_TABLE_CACHE_GET);
    // RecordTick(db_statistics_, TTT);
    #ifndef NDEBUG
    std::cout << "file "<<f->fd.GetNumber()<< std::endl;
    #endif
    *status = table_cache_->Get(
    ...
    if (get_context.State() != GetContext::kNotFound &&
      ...
      get_context.get_context_stats_.num_found += 1;
      get_context.ReportCounters();
    }
  ...
  if (GetContext::kMerge == get_context.State()) {
    get_context.get_context_stats_.num_merge += 1;
    if (!do_merge) {
      *status = Status::OK();
      return;
    }
    ...
    *status = MergeHelper::TimedFullMerge(
    ...
    get_context.get_context_stats_.num_not_found += 1;
    
    *status = Status::NotFound(); // Use an empty error message for speed
  }
  // 把report逻辑放到最后 是为了num_merge和num_not_found的正常添加
  if (db_statistics_ != nullptr) {
    get_context.ReportCounters();
  }
}
...
void VersionStorageInfo::EstimateCompactionBytesNeeded(
  ...
  uint64_t bytes_compact_to_next_level = 0; // 表示lsm-tree结构稳定情况下,本level总计的应该被compaction到下一个level的大小
  uint64_t level_size = 0;
  for (auto* f : files_[0]) {
  ...
  for (int level = base_level(); level <= MaxInputLevel(); level++) {
    level_size = 0; // 表示到当前level文件大小+上一level要达到target的应该减少的文件大小
    ...
    if (level_size > level_target) {
      ...
      if (bytes_next_level > 0) {
        assert(level_size > 0);
        // bytes_compact_to_next_level 就是当前level想要达到lsm-tree的平衡，需要往下一level compaction的字节数(注意，这个字节数已经考虑过前面的level都达到平衡，且把所有数据全都compaction到本层的情况了)
        // bytes_next_level 就是实际的下一level的字节数 level_size就是本level以上所有level都达到平衡之后，本level的实际字节个数
        // bytes_next_level/level_size就是当本level以上的所有level都达到平衡之后，下一level和本level的数据差异，这个数*bytes_compact_to_next_level就是本level想要达到平衡需要涉及到的下一level字节数，再加上
        // bytes_compact_to_next_level本身，就是本level想要达到平衡涉及的本level字节数，加起来就是总字节数
...
void VersionStorageInfo::ComputeCompactionScore(
    const ImmutableOptions& immutable_options,
    const MutableCFOptions& mutable_cf_options) {
  ComputeGCScore(immutable_options);
  ComputeVCscore(immutable_options);
  ...
  // 1.首先遍历每个level，计算每个level的score
  for (int level = 0; level <= MaxInputLevel(); level++) {
    double score;
    // 1.1 l0基于文件的个数计算score(很自然，l0文件不重叠，读取会涉及到太多的merge，自然越少越好)
    if (level == 0) {
      ...
      // 计算l0当前未在compaction中的文件个数num_sorted_runs.
      for (auto* f : files_[level]) {
      ...
      // 1.2 非l0，基于本层非进行compaction文件的总大小计算score
      ...
      if (!immutable_options.level_compaction_dynamic_level_bytes ||
        ...
        // 这就是默认的逻辑，计算当前level的非compaction的file_size和最大file_size的比值
...
//gc_score越大越应该被选中
bool gc_score_sort(const std::pair<uint32_t,std::pair<double,std::set<uint32_t>>> a, const std::pair<uint32_t,std::pair<double,std::set<uint32_t>>> b) {
    return a.second.first>b.second.first;
}

void VersionStorageInfo::ComputeGCScore(const ImmutableOptions& immutable_options) {
  double gc_score;
  // 1. 进行为优化空间放大而触发的gc compaction的选择
  for (auto m : table_files_) {
    FileMetaData* f = m.second;
    // 1.1 针对当前Version的每个table file,计算gc_score
    // gc_score设置为garbage_threshold/有效key占比 当有效key占比==threshold的时候，gc_score正好为1。当有效key占比<threshold的时候，gc_score大于1
    gc_score = immutable_options.gc_garbage_threshold/(f->reference_entries_/(double)f->total_entries_);
    // 如果当前table没有到达gc阈值，那么直接不进行任何处理
    if( gc_score < 1.0) continue;
    uint32_t table_number = f->fd.GetNumber();
    // 1.2 遍历当前table file的指向的所有index file
    for(auto m2 : f->fd.GetFatherNumberToReferencekey()){
      uint64_t index_number = m2.first;
      // 如果当前file处于compaction状态，则跳过，不会把它添加到index_to_score_tables_sort中，进而不会触发NeedsCompaction
      // WQTODOIMP 0426段错误原因 这里会返回nullptr...暂不知道why
      if(!GetFileMetaDataByNumber(index_number) || GetFileMetaDataByNumber(index_number)->being_compacted) continue;
      auto it = index_to_score_tables_for_spacegc.find(index_number);
      // 1.2.1 如果当前index file从未被添加，那么添加它，同时设置它的score和初始的table
      if(it == index_to_score_tables_for_spacegc.end()) {
        std::set<uint32_t> tables = {table_number};
        index_to_score_tables_for_spacegc.insert({index_number,{gc_score,tables}});
      }
      // 1.2.2 如果曾经被添加过，则更新它的score，同时添加table
      else {
        double now_gc_score = it->second.first;
        // 为什么这里index的score选用其引用的具有最小gc_score的table？因为我们期望这里遵循木桶效应。一个index的score应该取决于其指向的具有最少引用的table，才可以尽快把一个具有较少有效key的table gc掉。
        if( now_gc_score > gc_score) {
          it->second.first = now_gc_score;
        }
        it->second.second.insert(table_number);
      }
    }
  } 
  // // 2. 利用gc_score进行对因为空间放大而触发compaction的index排序
  // index_to_score_tables_for_spacegc_sort.resize(index_to_score_tables_for_spacegc.size());
  // for(auto m:index_to_score_tables_for_spacegc) {
  //   index_to_score_tables_for_spacegc_sort.push_back({m.first,m.second});
  // }
  // 2.1 利用gc_score进行对因为空间放大而触发compaction的index排序
  // std::sort(index_to_score_tables_for_spacegc_sort.begin(),index_to_score_tables_for_spacegc_sort.end(),gc_score_sort);
  // 3. 进行为优化范围查询而触发的gc compaction的选择
  for (int level = 0; level < num_levels(); level++) {   // 此处从level0开始也无所谓，level0的index的引用table大概率不会很高...
    const std::vector<FileMetaData*> level_files = files_[level];
    for(auto f: level_files) {
      uint32_t index_number = f->fd.GetNumber();
      // 3.1 如果当前index的引用table个数大于阈值，将该index和table添加到候选中
      const std::map<uint32_t,uint32_t>& sub_number_to_reference_key = f->fd.sub_number_to_reference_key;
      if(sub_number_to_reference_key.size() >= immutable_options.gc_range_tables_threshold) {
        // 3.1.1 计算得到当前index引用table的总个数 以及引用table的number和key的集合
        for( auto m: sub_number_to_reference_key) {
          gc_index_to_refs_for_rangegc[index_number] += m.second;
          index_to_refs_tables_for_rangegc[index_number].insert({m.second, m.first});
        }
      }
    }
  }
  // 3. 针对为了范围查询优化而选择的gc index进行其gc table的选择(不会选择该index的所有table进行real value compaction)
  for(auto& m:index_to_refs_tables_for_rangegc) {
    uint32_t index_number = m.first;
    // 如果当前file处于compaction状态，则跳过，不会把它添加到index_to_score_tables_sort中，进而不会触发NeedsCompaction
    if(GetFileMetaDataByNumber(index_number)->being_compacted) continue;
    uint32_t total_references = gc_index_to_refs_for_rangegc[index_number];
    std::set<uint32_t> chosen_tables;
    uint32_t temp_references = 0,last_references = 0;
    // 遍历当前index对应的table列表(table按照引用key个数从小到大排序)
    for(auto m2:m.second){
      uint32_t references = m2.first,table_number = m2.second;
      assert(last_references <= references); // references应该是从小到大排序的
      temp_references += references;
      chosen_tables.insert(table_number);
      // 退出选择table在两种条件都为真的情况下触发
      // 条件1 当前已选择的table的ref的总数到达阈值 
      // 条件2 当前正在选择的table的ref和上一个选择的ref的倍数相差超过一定值
      if(temp_references >= total_references*immutable_options.pick_table_threshold && references >= last_references * immutable_options.pick_table_max_multiple){
        break;
      }
      last_references = references;
    }
    double gc_range_score = chosen_tables.size()/(double)immutable_options.pick_index_min_table_numbers;
    if(gc_range_score <= 1) continue;
    index_to_score_tables_for_rangegc.push_back({index_number,{gc_range_score,chosen_tables}});
  }
  // // 4. 利用gc_score进行对因为范围查询而触发compaction的index排序
  // std::sort(index_to_score_tables_for_rangegc_sort.begin(),index_to_score_tables_for_rangegc_sort.end(),gc_score_sort);

  // 5. 合并面两个排序的结果(如果有同一个table在两者中均存在，则score取最大值，tables取两者并集
  for(auto i : index_to_score_tables_for_spacegc) {
    uint32_t index_number = i.first;
    gc_score = i.second.first;
    std::set<uint32_t>& tables = i.second.second;
    auto it = index_to_score_tables_.find(index_number);
    // index一定是第一次出现，直接添加
    std::pair<double,std::set<uint32_t>> score_to_tables = std::make_pair(gc_score,tables);
    index_to_score_tables_.insert({index_number,score_to_tables});
  }

  for(auto i : index_to_score_tables_for_rangegc) {
    uint32_t index_number = i.first;
    double gc_score = i.second.first;
    std::set<uint32_t>& tables = i.second.second;
    auto it = index_to_score_tables_.find(index_number);
    // index第一次出现，直接添加
    if(it == index_to_score_tables_.end()) {
      std::pair<double,std::set<uint32_t>> score_to_tables = std::make_pair(gc_score,tables);
      index_to_score_tables_.insert({index_number,score_to_tables});
    }
    // index非第一次出现，需要取score最大值，取tables的并集
    else {
      std::set<uint32_t>& tables_set = it->second.second;
      double& now_score = it->second.first;
      // table取并集
      for(auto table: tables){
        tables_set.insert(table);
      }
      // score取最大值
      now_score = std::max(now_score,gc_score);
    }
  }  
  index_to_score_tables_sort_.resize(index_to_score_tables_.size());
  int idx = 0;
  for(auto i:index_to_score_tables_) {
    index_to_score_tables_sort_[idx] = i;
    idx++;
  }
  std::sort(index_to_score_tables_sort_.begin(),index_to_score_tables_sort_.end(),gc_score_sort);
  // int idx1 = 0,idx2 = 0;
  // int max1 = index_to_score_tables_for_spacegc_sort.size(), max2 = index_to_score_tables_for_rangegc_sort.size();
  // while(idx1 < max1 && idx2 < max2){
  //   while(idx1 < max1 && idx2 < max2 && index_to_score_tables_for_spacegc_sort[idx1].second.first >= index_to_score_tables_for_rangegc_sort[idx2].second.first){
  //     // 如果index不相同，则取score最大值
  //     index_to_score_tables_sort.push_back(index_to_score_tables_for_spacegc_sort[idx1]);
  //     idx1++;
  //   }
  //   while(idx1 < max1 && idx2 < max2 && index_to_score_tables_for_spacegc_sort[idx1].second.first < index_to_score_tables_for_rangegc_sort[idx2].second.first){
  //     index_to_score_tables_sort.push_back(index_to_score_tables_for_rangegc_sort[idx2]);
  //     idx2++;
  //   }
  //   if(idx1 == max1) {
  //     index_to_score_tables_sort.insert(index_to_score_tables_sort.end(),index_to_score_tables_for_rangegc_sort.begin()+idx2,index_to_score_tables_for_rangegc_sort.end());
  //   } else {
  //     index_to_score_tables_sort.insert(index_to_score_tables_sort.end(),index_to_score_tables_for_spacegc_sort.begin()+idx1,index_to_score_tables_for_spacegc_sort.end());
  //   }
  // }
}
bool vc_score_sort(const std::pair<uint64_t,double> a, const std::pair<uint64_t,double> b) {
    return a.second > b.second;
}

void VersionStorageInfo::ComputeVCscore(const ImmutableOptions& immutable_options) {
  for (int level = 0; level <= MaxInputLevel(); level++) {
    double vc_score = 0;
    for (auto* f : files_[level]) {
        // vc_score是file维度的，所以不需要跳过being_compated的file
        vc_score = (f->merge_entries_ / (double)f->total_entries_)/immutable_options.vc_merge_threshold;
        index_to_vc_score_[level].push_back(std::make_pair(f->fd.GetNumber(),vc_score));
    }
  std::sort(index_to_vc_score_[level].begin(),index_to_vc_score_[level].end(),vc_score_sort);
  }
...
// void VersionStorageInfo::ChangeTableReference(int changes,uint32_t file_number){
//   assert(table_files_->count(file_number));
// }

void VersionStorageInfo::AddTableFile(FileMetaData* f){
  // 判断本次添加的table_file没有在历史版本出现过 如果assert失败，说明代码有bug
  uint32_t file_number = f->fd.GetNumber();
  assert(!table_files_.count(file_number));
  table_files_.insert(std::make_pair(file_number,f));

  // 这里不需要ref++ TableFile的index_refs的语义已经不一样了
}

void VersionStorageInfo::AddGuard(int level,std::string guard){
  auto& level_guards = guard_[level];
  // 有此行逻辑是因为，在SaveGuardsTo函数中，如果遇到了add_guard和based_guard中有相同的Guard，我们是无法通过cmp来剔除的。
  // 但是由于它们是按照非递减顺序来的，所以我们可以每次添加的时候都和guard中的最后一个元素比较，避免添加重复元素
  if(level_guards.empty() || level_guards.back() != guard)
    level_guards.push_back(guard);
}
...
void Version::AddLiveFiles(std::vector<uint64_t>* live_index_files,std::vector<uint64_t>* live_table_files,
                           std::vector<uint64_t>* live_blob_files) const {
  assert(live_index_files);
  ...
  for (int level = 0; level < storage_info_.num_levels(); ++level) {
    ...
    for (const auto& meta : level_files) {
      ...
      live_index_files->emplace_back(meta->fd.GetNumber());
    }

  }
  for (auto f:storage_info_.AllTableFiles()) {
      assert(f.second);

      live_table_files->emplace_back(f.second->fd.GetNumber());
...
void VersionSet::AddLiveFiles(std::vector<uint64_t>* live_index_files, std::vector<uint64_t>* live_table_files,
                              std::vector<uint64_t>* live_blob_files) const {
  assert(live_index_files);
  assert(live_table_files);
  assert(live_blob_files);
  ...
  for (auto cfd : *column_family_set_) {
    ...
    for (Version* v = dummy_versions->next_; v != dummy_versions;
      ...
      for (int level = 0; level < vstorage->num_levels(); ++level) {
        total_index_files += vstorage->LevelFiles(level).size();
      }

      total_table_files += vstorage->AllTableFiles().size();
  ...
  live_index_files->reserve(live_index_files->size() + total_index_files);
  ...
  for (auto cfd : *column_family_set_) {
    ...
    for (Version* v = dummy_versions->next_; v != dummy_versions;
         v = v->next_) {
      v->AddLiveFiles(live_index_files, live_table_files, live_blob_files);
      if (v == current) {
        found_current = true;
      }
    }
    ...
    if (!found_current && current != nullptr) {
      // Should never happen unless it is a bug.
      assert(false);
      current->AddLiveFiles(live_index_files, live_table_files, live_blob_files);
    }
...
InternalIterator* VersionSet::MakeInputIterator(
  ...
  // 1.获取Iter所需的空间：l0文件个数+非l0的输入层数
  // 比如本次compaction是 
  // l0 1 5 9
  // l1 2 55 66   那么总空间就是3+1=4
  // 比如本次compaction是
  // l1 5 9 12
  // l2 551 214 12 那么总空间就是1+1=2
...
void VersionSet::GetObsoleteFiles(std::vector<ObsoleteFileInfo>* files,std::vector<ObsoleteFileInfo>* table_files,
  ...
  std::vector<ObsoleteFileInfo> pending_table_files;
  for (auto& f : obsolete_table_files_) {
    if (f.metadata->fd.GetNumber() < min_pending_output) {
      table_files->emplace_back(std::move(f));
    } else {
      pending_table_files.emplace_back(std::move(f));
    }
  }
  obsolete_table_files_.swap(pending_table_files);
