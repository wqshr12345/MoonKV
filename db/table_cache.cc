...
Status TableCache::GetTableReader(
    ...
    size_t max_file_size_for_l0_meta_pin, Temperature file_temperature,bool is_table) {
  // 只有在访问NvmTable的情况下，构造RandomAccessFileReader才是有意义的
  // 如果当前是NvmPartition，那么这里不需要提前构造任何东西，下面留一个空的RandomAccessFileReader作为入参，这个参数之后也不会被用到。
  if(is_table){
    std::string fname =
    ...
    // 根据is_table 判断此处应该返回NvmPartiton还是NvmTable
    s = ioptions_.second_table_factory->NewTableReader(
        ro,
        TableReaderOptions(
    ...
    return s;
      ...
      std::unique_ptr<RandomAccessFileReader> file_reader;
      Status s = static_cast<NvmPartitionFactory*>(ioptions_.table_factory.get())->NewTableReader2(
          ro,
          TableReaderOptions(
              ioptions_, prefix_extractor, file_options, internal_comparator,
              skip_filters, immortal_tables_, false /* force_direct_prefetch */,
              level, fd.largest_seqno, block_cache_tracer_,
              max_file_size_for_l0_meta_pin, db_session_id_, fd.GetNumber()),
          std::move(file_reader), table_reader,this,
          prefetch_index_and_filter_in_cache);
...
Status TableCache::FindTable(
  ...
  *handle = cache_->Lookup(key);
  ...
  // 双nullptr的判断，避免每次都要上锁。
  if (*handle == nullptr) {
    if (no_io) {
      return Status::Incomplete("Table not found in table_cache, no_io is set");
    }
    MutexLock load_lock(loader_mutex_.get(key,is_table));
    ...
    *handle = cache_->Lookup(key);
    ...
    Status s = GetTableReader(
        ...
        max_file_size_for_l0_meta_pin, file_temperature,is_table);
    if (!s.ok()) {
...
InternalIterator* TableCache::NewIterator(
  ...
  Status s;
  // 1.获取当前FileMetaData中的table_reader。如果为空，则调用FindTable重新构造一个。
...
Status TableCache::Get(
    ...
    size_t max_file_size_for_l0_meta_pin) {
  auto& fd = file_meta.fd;
  // 如果key大于当前fd的最大key 提前退出(目前暂不清楚为什么key大于最大key，依然会进入这个函数)
  if(ioptions_.user_comparator->Compare(ExtractUserKey(k), file_meta.largest.user_key()) > 0) {
    return Status::OK();
  }
  ...
  if (!done) {
    ...
    if (s.ok()) {
      ...
      // 为当前查询的table设置children_ranks
      // get_context->set_last_children_ranks();
      // get_context->set_children_ranks(file_meta.children_ranks_);
      // get_context->set_last_level();
      get_context->set_level(level);
      get_context->set_last_file_number();
      get_context->set_file_number(fd.GetNumber());
      // get_context->set_total_entries(file_meta.total_entries_);
      StopWatch* sw = new StopWatch(ioptions_.clock, ioptions_.stats, DB_TABLE_GET);
      s = t->Get(options, k, get_context, prefix_extractor.get(), skip_filters, ioptions_);
      delete sw;
