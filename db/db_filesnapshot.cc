...
Status DBImpl::GetLiveFiles(std::vector<std::string>& ret,
  ...
  *manifest_file_size = 0;
  ...
  std::vector<uint64_t> live_index_files;
  ...
  for (auto cfd : *versions_->GetColumnFamilySet()) {
    ...
    cfd->current()->AddLiveFiles(&live_index_files, &live_table_files, &live_blob_files);
  }

  ret.clear();
  ret.reserve(live_index_files.size() + live_table_files.size() + live_blob_files.size() +
              3);  // for CURRENT + MANIFEST + OPTIONS

  // create names of the live files. The names are not absolute
  // paths, instead they are relative to dbname_.
  for (const auto& index_file_number : live_index_files) {
    ret.emplace_back(MakeIndexFileName("", index_file_number));
  }
