...
WriteStallCondition ColumnFamilyData::RecalculateWriteStallConditions(
  ...
  if (current_ != nullptr) {
    ...
    // 通过GetWriteStallConditionAndCause()方法,计算得到当前cfd的一个condition.根据不同
    // 的condition,再通过调用write_controller的GetStopToken()或者GetDelayToken(),增加
    // 在write_controller内部的一些变量.同时write_controller是cfd和dbimpl的共享的指针对象
    // 进而在dbimpl write的时候进行阻塞。
