...
Status MemTableList::TryInstallMemtableFlushResults(
  ...
  for (size_t i = 0; i < mems.size(); ++i) {
    ...
    // 将本次flush涉及到的memtable的flush_completed_置为true
  ...
  while (s.ok()) {
    ...
    // 从后往前遍历是为了从老到新遍历 确保先把老的memtable的flush信息添加到下一version
    for (auto it = memlist.rbegin(); it != memlist.rend(); ++it) {
      MemTable* m = *it;
      ...
      if (it == memlist.rbegin() || batch_file_number != m->file_number_) {
        ...
        // 如果最老的那个memtable很慢才flush完成，那么所有后续的flush完成的table的VersionEdit会积压在这里，存入edit_list中
