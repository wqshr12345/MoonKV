  ...
  *made_progress = false;
  ...
  if (is_manual) {
    ...
    if (!mutable_cf_options->disable_auto_compactions && !cfd->IsDropped()) {
      ...
      // 2.选择本次Compaction的compaction(即input file和output file) 这是在大锁mutex_的保护下选择的，不用担心线程同步问题
      // std::cout<<"pre pick compaction"<<std::endl;
      c.reset(cfd->PickCompaction(*mutable_cf_options, mutable_db_options_,
                                  log_buffer));
      if(c == nullptr){
        std::cout<<"Pick Compaction Null!"<<std::endl;
      } else{
        std::cout<<"Pick Compaction Not Null!"<<std::endl;
        std::string compaction_type = c->is_gc_compaction() ? "gc" :(c->is_vertical_compaction() ? "vertical" : "normal");
        std::cout<<"Compaction Type : "<<compaction_type<<std::endl;
        // std::cout<<"is_gc_compaction "<<c->is_gc_compaction()<<std::endl;
        // std::cout<<"is_vertical_compaction "<<c->is_vertical_compaction()<<std::endl;
        std::cout<<"Pick Files: "<<std::endl;
        for(auto& l:*c->inputs()){
        ...
        if(c->is_gc_compaction()){
          std::cout<<"Gc Index To Tables: "<<std::endl;
          for(auto m:c->get_index_to_tables()) {
            std::cout<<m.first<<"[";
            for(auto t:m.second){
              std::cout<<t<<" ";
            }
    ...
    *made_progress = true;
    ...
    for (unsigned int l = 0; l < c->num_input_levels(); l++) {
      ...
      for (size_t i = 0; i < c->num_input_files(l); i++) {
        ...
        c->edit()->AddFile(
            c->output_level(), f->fd.GetNumber(), f->fd.GetPathId(),f->children_ranks_, f->total_entries_,f->merge_entries_,
            f->fd.GetSubNumberToReferencekey(),
  ...
  // 经过一系列else之后，最后才是最主要的compaction执行过程——构造CompactionJob
    TEST_SYNC_POINT_CALLBACK("DBImpl::BackgroundCompaction:BeforeCompaction",
                             c->column_family_data());
    ...
    CompactionJob compaction_job(
        ...
        &bg_bottom_compaction_scheduled_);
    compaction_job.first_pick_guard_ = &this->first_pick_guard_;
    compaction_job.Prepare();
    ...
    // Should handle erorr?
    // 执行compaction job是可以并发进行的，这里没有mutex_的lock
    compaction_job.Run().PermitUncheckedError();
    TEST_SYNC_POINT("DBImpl::BackgroundCompaction:NonTrivial:AfterRun");
    mutex_.Lock();
    
    std::cout<<"This Compaction"<<std::endl;
    std::string compaction_type = c->is_gc_compaction() ? "gc" :(c->is_vertical_compaction() ? "vertical" : "normal");
    std::cout<<"Compaction Type : "<<compaction_type<<std::endl;
    std::cout<<"Compaction Guard : [";
    for( auto guard: compaction_job.boundaries_) {
      std::cout<<guard<<" ";
    }
    std::cout<<"]"<<std::endl;
    // std::cout<<"is_gc_compaction "<<c->is_gc_compaction()<<std::endl;
    ...
    // std::cout<<"is gc compaction ? "<<c->is_gc_compaction()<<std::endl;
    // std::cout<<"---index file---"<<std::endl;
    // for(auto& l:*c->inputs()){
    //   std::cout<<"level "<<l.level<<" : ";
    //   for(auto& f:l.files){
    //     std::cout<<" files "<<f->fd.GetNumber();
    //   }
    //   std::cout<<std::endl;
    // }
    //   std::cout<<std::endl;
    ...
    std::cout<<"Version Before Compaction"<<std::endl;
    std::cout<<"Guard"<<std::endl;
    for(int i = 1;i<4;i++) {
      std::cout<<"level "<<i<<" : [";
      for(auto& g:c->column_family_data()->current()->storage_info()->UpGuard(i)) {
        std::cout<<g<<" ";
      }
      std::cout<<"]"<<std::endl;
    }
    ...
    for(int i = 0;i<7;i++){
      ...
      std::cout<<"level "<<i<<":";
      std::cout<<"total "<<c->column_family_data()->current()->storage_info()->LevelFiles(i).size()<<" ";
      for(auto& f:c->column_family_data()->current()->storage_info()->LevelFiles(i)){
      ...
      std::cout<<std::endl;
      // std::cout<<" level size "<<file_size<<" level score "<<c->column_family_data()->current()->storage_info()->CompactionScore(i)<<std::endl;
    }
    // std::cout<<"table files "<<std::endl;
    // for(auto& f:c->column_family_data()->current()->storage_info()->AllTableFiles()){
    ...
    std::cout<<"Version After Compaction"<<std::endl;
    std::cout<<"Guard"<<std::endl;
    for(int i = 1;i<4;i++) {
      std::cout<<"level "<<i<<" : [";
      for(auto& g:c->column_family_data()->current()->storage_info()->UpGuard(i)) {
