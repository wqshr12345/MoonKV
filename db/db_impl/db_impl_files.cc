...
void DBImpl::FindObsoleteFiles(JobContext* job_context, bool force,
  ...
  versions_->GetObsoleteFiles(
      &job_context->index_delete_files, &job_context->table_delete_files, &job_context->blob_delete_files,
      &job_context->manifest_delete_files, job_context->min_pending_output);

  // Mark the elements in job_context->index_delete_files and
  ...
  for (const auto& index_to_del : job_context->index_delete_files) {
    MarkAsGrabbedForPurge(index_to_del.metadata->fd.GetNumber());
  }

  for (const auto& table_to_del : job_context->table_delete_files) {
    MarkAsGrabbedForPurge(table_to_del.metadata->fd.GetNumber());
  }
  ...
  if (doing_the_full_scan) {
    versions_->AddLiveFiles(&job_context->index_live, &job_context->table_live, &job_context->blob_live);
    ...
    versions_->RemoveLiveFiles(job_context->index_delete_files,
...
void DBImpl::DeleteObsoleteFileImpl(int job_id, const std::string& fname,
  ...
  if (type == kTableFile || type == kBlobFile || type == kWalFile || type == kValueFile) {
...
// files in index_delete_files and log_delete_files.
...
void DBImpl::PurgeObsoleteFiles(JobContext& state, bool schedule_only) {
  ...
  std::unordered_set<uint64_t> index_live_set(state.index_live.begin(),
                                            state.index_live.end());
  std::unordered_set<uint64_t> table_live_set(state.table_live.begin(),
                                            state.table_live.end());
  ...
  candidate_files.reserve(
      candidate_files.size() + state.index_delete_files.size() +
      state.blob_delete_files.size() + state.log_delete_files.size() +
      state.manifest_delete_files.size());
  // We may ignore the dbname when generating the file names.
  for (auto& file : state.index_delete_files) {
    if (!file.only_delete_metadata) {
      candidate_files.emplace_back(
          MakeIndexFileName(file.metadata->fd.GetNumber()), file.path);
    }
  ...
  std::cout<<"Delete Table Meta :["<<std::endl;
  for (auto& file : state.table_delete_files) {
    uint32_t number = file.metadata->fd.GetNumber();
    std::cout<<number<<" ";
    candidate_files.emplace_back(
        MakeTableFileName(number), file.path);
        // WQTODOIMP 暂时不delete table file的meta...这可能会有内存泄漏 但是如果在这里delete 后续在version的析构里面，遍历table的时候就找不到这个meta了
        // 本质上还是自己实现的ref可能有bug...应该用Blob来开发，会方便特别多...
    // file.DeleteMetadata();
  }
  std::cout<<"]"<<std::endl;
  ...
  for (const auto& candidate_file : candidate_files) {
    ...
    switch (type) {
      ...
      case kTableFile:
        ...
        keep = (index_live_set.find(number) != index_live_set.end()) ||
      ...
      case kTempFile:
        ...
        keep = (index_live_set.find(number) != index_live_set.end()) ||
               (table_live_set.find(number) != table_live_set.end()) ||
               (blob_live_set.find(number) != blob_live_set.end()) ||
    ...
    // index file和table file都是直接从这里得到fname
    if (type == kTableFile || type == kValueFile) {
      // evict from cache
      TableCache::Evict(table_cache_.get(), number);
      // fname = MakeTableFileName(candidate_file.file_path, number);
      // 这里不能简单生成index file name 因为table和index都会走到这里的逻辑
      // fname = MakeIndexFileName(candidate_file.file_path,number);
      fname = candidate_file.file_path + "/" + candidate_file.file_name;
