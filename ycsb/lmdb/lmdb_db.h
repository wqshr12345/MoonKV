...
class LmdbDB : public DB {
 public:
  ...
  Status Merge(const std::string &table, const std::string &key, std::vector<Field> &values) {
    return (this->*(method_merge_))(table, key, values);
  }
 ...
 private:
  ...
  Status (LmdbDB::*method_merge_)(const std::string &, const std::string &, std::vector<Field> &);
