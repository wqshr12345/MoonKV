...
enum Operation {
  ...
  READMODIFYWRITE,
  DELETE,
  MERGE, // 为什么要加入这个枚举？因为ycsb里面实现的READMODIFYWRITE并不是调的Merge，而是先Read再Merge......
  MAXOPTYPE
};
...
class CoreWorkload {
 public:
  ...
  static const std::string MERGE_PROPORTION_PROPERTY;
  static const std::string MERGE_PROPORTION_DEFAULT;
  ...
  virtual bool DoTransactionInOrder(DB &db, double ops_percent);
...
inline bool CoreWorkload::DoTransactionInOrder(DB &db, double ops_percent) {
  int status = -1;
  switch (op_chooser_.GetValueFromOps(ops_percent)) {
    case MERGE:
      status = TransactionMerge(db);
      break;
    case READ:
...
inline int CoreWorkload::TransactionMerge(DB &db) {
  ...
  return db.Merge(table_name_, key, values);
}
...
// 弃用 这个语义不明不白的 先在这里read一遍，然后再Update那里，还会再读一遍，莫名其妙。
