...
const string CoreWorkload::MERGE_PROPORTION_PROPERTY = "mergeproportion";
const string CoreWorkload::MERGE_PROPORTION_DEFAULT = "0.0";
...
void CoreWorkload::Init(const utils::Properties &p) {
  ...
  double merge_proportion = std::stod(p.GetProperty(MERGE_PROPORTION_PROPERTY,
                                                    MERGE_PROPORTION_DEFAULT));
  ...
  // WQTODO 暂时在这里改顺序 后续考虑能不能从文件读顺序呢？
  if(merge_proportion > 0) {
    op_chooser_.AddValue(MERGE, merge_proportion);
  }
