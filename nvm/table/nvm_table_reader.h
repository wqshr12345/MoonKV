...
class NvmTable : public TableReader {
 public:
 ...
 protected:
  Rep* rep_;
  explicit NvmTable(Rep* rep, BlockCacheTracer* const block_cache_tracer)
      : rep_(rep), block_cache_tracer_(block_cache_tracer) {}
  // No copying allowed
  explicit NvmTable(const TableReader&) = delete;
 ...
 private:
  ...
  // func: 本方法只在NvmTable中有用，而且只在block_align的情况下有效。
  // 给定offset，找到这个offset所在block的offset和size
  BlockHandle GetBlockHandleFromOffset(const ReadOptions& read_options,const uint32_t& offset,uint32_t& block_offset);
  ...
  Status PrefetchIndexAndFilterBlocks(
      ...
      InternalIterator* meta_iter, NvmTable* new_table,
...
class NvmTable::PartitionedIndexIteratorState
 ...
 private:
  // Don't own table_
  const NvmTable* table_;
  UnorderedMap<uint64_t, CachableEntry<Block>>* block_map_;
};

// Stores all the properties associated with a NvmTable.
// These are immutable.
struct NvmTable::Rep {
  ...
  std::size_t ApproximateMemoryUsage() const {
    ...
    usage += malloc_usable_size(const_cast<NvmTable::Rep*>(this));
#else
    usage += sizeof(*this);
...
class WritableFileStringStreamAdapter2 : public std::stringbuf {
 public:
  explicit WritableFileStringStreamAdapter2(WritableFile* writable_file)
      : file_(writable_file) {}
