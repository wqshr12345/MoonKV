...
constexpr size_t kBlockTrailerSize = NvmTable::kBlockTrailerSize;
...
// Unused 全局函数，在BLockBasedTable中已有定义
...
const uint64_t kNvmTableMagicNumber = 0x88e241b785f4cff7ull;
// We also support reading and writing legacy block based table format (for
// backwards compatibility)
const uint64_t kLegacyNvmTableMagicNumber = 0xdb4775248b80fb57ull;
...
class NvmTableBuilder::BlockBasedTablePropertiesCollector
...
struct NvmTableBuilder::Rep {
  ...
  uint64_t value_offset; // 本offset用来存储当前value的offset。为什么不用上面这个offset？上面的offset仅在block被flush才更新，实时性不够。而且这里没有用atomic的必要。
  size_t alignment;
  NvmDataBlockBuilder data_block;
  ...
  uint64_t get_value_offset() { return value_offset; }
  void add_value_offset(uint64_t add_offset){ value_offset += add_offset; }
  void set_value_offset(uint64_t o) { value_offset = o; }

  // Noused: 默认不会开启，所以我们的代码中不会做相应适配
      ...
      : ioptions(tbo.ioptions),
        ...
        flush_block_policy(
            table_options.flush_block_policy_factory->NewFlushBlockPolicy(
                table_options, data_block)), // 这里已经修改了其中的newFlushBlockPolicy函数，其可以根据NvmDataBlockBuilder构造一flush_block_policy，且策略也会变化。
        status_ok(true),
        io_status_ok(true) {
...
// Noused
struct NvmTableBuilder::ParallelCompressionRep {
  ...
  // and NvmTableBuilder::FileSize() is no longer accurate.
...
NvmTableBuilder::NvmTableBuilder(
...
// 本接口在NvmTableBuilder中无用。因为Table中无需接收key
void NvmTableBuilder::Add(const Slice& key, const Slice& value) {
  return;
}

// func: 本函数用来将一个<key,value>构造在内存中。如果到达指定条件，会调用flush()将其刷入持久存储
uint64_t NvmTableBuilder::Add2(const Slice& value){
  Rep* r = rep_;
  assert(rep_->state != Rep::State::kClosed);
  uint64_t offset = 0;
  if (!ok()) return offset;
  const Slice key("nvm_rocksdb"); // 无用key 仅仅用来占位
  //ValueType value_type = ExtractValueType(key);
  // 注意，此处的value_type仅有RangeDeletion和非RangeDeletion两种
  // 我们的实现中不会为RangeDeletion单独维护一个block，甚至不会加入针对它的测试
  // 所以，统一用TypeValue代替
  ValueType value_type = ValueType::kTypeValue;
  // 1.如果key的类型是value_type(包括Value、Merge、Deletion等)，则将其构造到data block中。
  if (IsValueType(value_type)) {
#ifndef NDEBUG
    // 1.1 比较本次key和last key的大小，应该满足严格增序。
    // WQTODO 这个放到更外层Table做
    // if (r->props.num_entries > r->props.num_range_deletions) {
    //   assert(r->internal_comparator.Compare(key, Slice(r->last_key)) > 0);
    // }
#endif  // !NDEBUG
    // 1.2 判断本次是否应该将数据flush到磁盘(一般是数据到了某个block的大小) WQTODO 未来应该会为index和value都做should_flush的逻辑. 当然，这里可能有并发问题，比如某个index持久化成功而value没有持久化成功，到时候再来解决这个问题。
    // WQTODO 是否需要维护两个线程，分别写Nvm的Index和SSD的Data呢？也许是一个小小的优化
    // 这里的Update(key,value)中的key是无用的，保留这个入参只是为了兼容以前的接口。
    ...
    //   }
    // }
    // 1.4 将当前的value加入到data block中，然后增加当前的value_offset 同时将添加value前的offset返回
    //  WQTODO 将当前的index加入到nvm block中
    offset = r->get_value_offset();
    r->add_value_offset(r->data_block.AddValue(value));
    ...
    // WQTODO 这个函数是干嘛的不是很清楚...
    NotifyCollectTableCollectorsOnAdd(key, value, r->get_offset(),
                                      r->table_properties_collectors,
                                      r->ioptions.logger);
  } 
  // Noused
  // 2.如果key的类型是RangeDeletion，那么将其构造到RangeDeletion block中。 我们的基准测试中不会有这样的情况出现，所以这里的逻辑应该永远不会被调用，这里的代码我们暂时不做任何处理。
  ...
  // 3.出现其他value type，则说明可能数据损坏，直接提前assert暴露错误。
  else {
    assert(false);
  }
  // 4.添加(key,value)过程中的一些统计信息的添加
  r->props.num_entries++;
...
// func:本函数用于将当前内存中的index和value刷入持久存储中
void NvmTableBuilder::Flush() {
  Rep* r = rep_;
  assert(rep_->state != Rep::State::kClosed);
  if (!ok()) return;
  if (r->data_block.empty()) return;
  // Noused: 我们不会开启ParallelCompression，所以下面逻辑不会触发。
  // 1.如果开启了parallelCompression，会进行一些多线程压缩技术
  // if (r->IsParallelCompressionEnabled() &&
  ...
  // } 
  // 否则，直接将data_block的内容刷入生磁盘
  else {
    WriteBlock(&r->data_block, &r->pending_handle, BlockType::kData);
  }
}

void NvmTableBuilder::WriteBlock(NvmDataBlockBuilder* block,
                                        BlockHandle* handle,
...
void NvmTableBuilder::WriteBlock(const Slice& raw_block_contents,
                                        BlockHandle* handle,
                                        BlockType block_type) {
  Rep* r = rep_;
  ...
  // 1.压缩数据 
  // Question: 压缩之后的大小已经小于一个block size了，但还是按照一个block去写入，这样不会有问题吗？还是说在FlushPolicy那里有判断？
...
// Noused
void NvmTableBuilder::BGWorkCompression(
...
void NvmTableBuilder::CompressAndVerifyBlock(
...
void NvmTableBuilder::WriteRawBlock(const Slice& block_contents,
                                           CompressionType type,
                                           BlockHandle* handle,
                                           BlockType block_type,
  ...
  // 在这里确定本次写入磁盘的block的offset和size
  // WQTODO 这里有一个很大的问题，就是说RocksDB的block_align是默认不开启的
  // 如果我们的设计中也不使用block_align，有一个很大的问题就是我们需要仿照RocksDB的index block设计[offset,size]，存在Nvm或者Ssd中，
  // 查询的时候，先去那里面找到当前value是在哪一个block中，然后再把这个block读出来。而无法根据B-Tree的index直接获得block
  // 如果使用了block_align，则不需要这个额外的存储，直接根据B-Tree的index就可以获得block
  ...
  {
  // 1.与文件系统交互，将block_contents内容放入磁盘
  ...
  trailer[0] = type;
  // 2.计算本block_contents的checksum，以便后续校验
  ...
  // 3.将checksum写入trailer，然后与文件系统交互，将后者写入磁盘。
  EncodeFixed32(trailer.data() + 1, checksum);
  TEST_SYNC_POINT_CALLBACK(
      "NvmTableBuilder::WriteRawBlock:TamperWithChecksum",
      trailer.data());
  {
...

  {
    ...
    switch (r->table_options.prepopulate_block_cache) {
  	...
  	// 4.如果设置了warm_cache,那么在将data block刷入磁盘之前，先将原始磁盘内容放到缓存中
    if (warm_cache) {
  ...
  // value_offset此处不必要加block_contents 因为已经在Add2函数中增加了
  r->add_value_offset(kBlockTrailerSize);
  // 注意，block_align是必须要打开的，不然我们的TableReader读取的时候会出错。
  // 5.如果设置了block_align且当前是data block,那么必须计算pad bytes，然后补上这些空白。
  // 注意，非data block是不会使用block_align策略的orz...
...
// Noused
void NvmTableBuilder::BGWorkWriteRawBlock() {
  Rep* r = rep_;
...
// Noused
void NvmTableBuilder::StartParallelCompression() {
  rep_->pc_rep.reset(
...
void NvmTableBuilder::StopParallelCompression() {
...
Status NvmTableBuilder::status() const { return rep_->GetStatus(); }

IOStatus NvmTableBuilder::io_status() const {
...
//
Status NvmTableBuilder::InsertBlockInCompressedCache(
  ...
  if (type != kNoCompression && block_cache_compressed != nullptr) {
    ...
    CacheKey key = NvmTable::GetCacheKey(rep_->base_cache_key, *handle);
...
Status NvmTableBuilder::InsertBlockInCacheHelper(
...
Status NvmTableBuilder::InsertBlockInCache(const Slice& block_contents,
                                                  const BlockHandle* handle,
                                                  BlockType block_type) {
  ...
  if (block_cache != nullptr) {
    ...
    if (s.ok()) {
      ...
      NvmTable::UpdateCacheInsertionMetrics(
...
void NvmTableBuilder::WriteFilterBlock(
  ...
  if (ok()) {
    ...
    while (ok() && s.IsIncomplete()) {
      ...
      // NvmTableBuilder is deallocated.
      //
  ...
  if (ok()) {
    ...
    // of filter data.
    std::string key;
    key = is_partitioned_filter ? NvmTable::kPartitionedFilterBlockPrefix
                                : NvmTable::kFullFilterBlockPrefix;
    key.append(rep_->table_options.filter_policy->CompatibilityName());
    meta_index_builder->Add(key, filter_block_handle);
  }
}

void NvmTableBuilder::WriteIndexBlock(
...
void NvmTableBuilder::WritePropertiesBlock(
    MetaIndexBuilder* meta_index_builder) {
  ...
  if (ok()) {
    ...
    TEST_SYNC_POINT_CALLBACK(
        "NvmTableBuilder::WritePropertiesBlock:BlockData", &block_data);
  ...
  if (ok()) {
#ifndef NDEBUG
    {
      ...
      TEST_SYNC_POINT_CALLBACK(
          "NvmTableBuilder::WritePropertiesBlock:GetPropsBlockOffset",
          &props_block_offset);
      TEST_SYNC_POINT_CALLBACK(
          "NvmTableBuilder::WritePropertiesBlock:GetPropsBlockSize",
          &props_block_size);
    }
#endif  // !NDEBUG
    ...
    TEST_SYNC_POINT_CALLBACK(
        "NvmTableBuilder::WritePropertiesBlock:Meta",
...
// Noused
void NvmTableBuilder::WriteCompressionDictBlock(
    MetaIndexBuilder* meta_index_builder) {
  if (rep_->compression_dict != nullptr &&
    ...
    if (ok()) {
      ...
      TEST_SYNC_POINT_CALLBACK(
          "NvmTableBuilder::WriteCompressionDictBlock:RawDict",
          &compression_dict);
#endif  // NDEBUG
    }
    if (ok()) {
...
// Noused
void NvmTableBuilder::WriteRangeDelBlock(
    MetaIndexBuilder* meta_index_builder) {
...
void NvmTableBuilder::WriteFooter(BlockHandle& metaindex_block_handle,
                                         BlockHandle& index_block_handle) {
  Rep* r = rep_;
  // this is guaranteed by NvmTableBuilder's constructor
...
// Noused: 因为我们不开启dictionary策略，所以这里的Buffered->Unbuffered的策略不会被触发.
void NvmTableBuilder::EnterUnbuffered() {
  Rep* r = rep_;
...
//func: 结束一个sst的构造。将filter block、meta block、footer等写入磁盘
Status NvmTableBuilder::Finish() {
  Rep* r = rep_;
  ...
  // Noused
  // 在finish的时候，也要为最后一个data block构造index 但在我们的逻辑中无用
//   if (r->IsParallelCompressionEnabled()) {
...
//   }
  ...
  // 注意，我们的设计中已经没有了indexblock，所以这个值无用
  // 在下面设置offset和size仅仅是为了兼容Footer的设计，它其中需要indexblock的handle。但一定要注意，这不是真实的！
  // 读取的时候也不要使用这个值，这里的4和25毫无意义...!
  index_block_handle.set_offset(4);
  index_block_handle.set_size(25);
  // meta_index_builder 用于构造一个metaindex block
  // 这个block中存储一系列key value。key是metablock的名字如"rocksdb.range_del"，value是 metablock的offset+size 但不包括index block的
  // index block的offset+size 在Footer中添加...
  MetaIndexBuilder meta_index_builder;
  // WQTODO 如果需要filter block 未来再加
  // WriteFilterBlock(&meta_index_builder);
  // WriteIndexBlock(&meta_index_builder, &index_block_handle);
  // Noused
  // WriteCompressionDictBlock(&meta_index_builder);
  // Noused 
  // 在我们的代码中，不会为RangeDel和compressionDictionary做单独适配，所以这两处代码的逻辑实际上不会走到。
...
uint64_t NvmTableBuilder::NumEntries() const {
  return rep_->props.num_entries;
}

bool NvmTableBuilder::IsEmpty() const {
  return rep_->props.num_entries == 0 && rep_->props.num_range_deletions == 0;
}

uint64_t NvmTableBuilder::FileSize() const { return rep_->offset; }

uint64_t NvmTableBuilder::EstimatedFileSize() const {
  if (rep_->IsParallelCompressionEnabled()) {
...
bool NvmTableBuilder::NeedCompact() const {
...
TableProperties NvmTableBuilder::GetTableProperties() const {
...
std::string NvmTableBuilder::GetFileChecksum() const {
...
const char* NvmTableBuilder::GetFileChecksumFuncName() const {
  if (rep_->file != nullptr) {
...
void NvmTableBuilder::SetSeqnoTimeTableProperties(
...
const std::string NvmTable::kObsoleteFilterBlockPrefix = "filter.";
const std::string NvmTable::kFullFilterBlockPrefix = "fullfilter.";
const std::string NvmTable::kPartitionedFilterBlockPrefix =
    "partitionedfilter.";
}  // namespace ROCKSDB_NAMESPACE
