...
void TailPrefetchStats2::RecordEffectiveSize(size_t len) {
  MutexLock l(&mutex_);
...
size_t TailPrefetchStats2::GetSuggestedPrefetchSize() {
...
NvmTableFactory::NvmTableFactory(
    const BlockBasedTableOptions& _table_options)
    : table_options_(_table_options) {
  InitializeOptions();
  RegisterOptions(&table_options_, &nvm_table_type_info);

  // WQTODO 这里的kBlockBasedTableReader暂且不进行替换
  // 因为它和原有代码的耦合度太高了，我没有功夫去捋顺那一坨代码...
...
void NvmTableFactory::InitializeOptions() {
...
Status NvmTableFactory::PrepareOptions(const ConfigOptions& opts) {
  InitializeOptions();
...
Status NvmTableFactory::NewTableReader(
    const ReadOptions& ro, const TableReaderOptions& table_reader_options,
    std::unique_ptr<RandomAccessFileReader>&& file, uint64_t file_size,
    std::unique_ptr<TableReader>* table_reader,
    bool prefetch_index_and_filter_in_cache) const {
  return NvmTable::Open(
...
TableBuilder* NvmTableFactory::NewTableBuilder(
    const TableBuilderOptions& table_builder_options,
    WritableFileWriter* file) const {
  return new NvmTableBuilder(table_options_, table_builder_options,
                                    file);
}

Status NvmTableFactory::ValidateOptions(
  ...
  if (!IsSupportedFormatVersion(table_options_.format_version)) {
    return Status::InvalidArgument(
        "Unsupported NvmTable format_version. Please check "
        "include/rocksdb/table.h for more info");
  }
  // Question: 为什么block_align不能和compression_type一起用？
...
std::string NvmTableFactory::GetPrintableOptions() const {
  std::string ret;
  ret.reserve(20000);
...
const void* NvmTableFactory::GetOptionsPtr(
...
//     "table_options".
Status NvmTableFactory::ParseOption(const ConfigOptions& config_options,
...
// }
// Status GeBlockBasedTableOptionsFromString(
...
//   NvmTableFactory bbtf(table_options);
...
TableFactory* NewNvmTableFactory(
    const BlockBasedTableOptions& _table_options) {
  return new NvmTableFactory(_table_options);
}
