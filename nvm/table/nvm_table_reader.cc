...
// clang-format off
// WQTODO 未来有机会弄这个async的东西...
// #define WITHOUT_COROUTINES
// #include "nvm/table/nvm_table_reader_sync_and_async.h"
...
BlockHandle NvmTable::GetBlockHandleFromOffset(const ReadOptions& read_options,const uint32_t& offset,uint32_t& block_offset){
  BlockHandle handle;
  block_offset = offset%read_options.block_size;
  // 注意，handle中的offset是block的offset。这么写是为了适配原有的BlockHandle接口。
  handle.set_offset(offset-block_offset);
  handle.set_size(read_options.block_size-NvmTable::kBlockTrailerSize);
  return handle;
}

void NvmTable::UpdateCacheHitMetrics(BlockType block_type,
...
void NvmTable::UpdateCacheMissMetrics(BlockType block_type,
...
void NvmTable::UpdateCacheInsertionMetrics(
...
// func: 根据key去cache中找一个具体的内容。如果存在，把句柄返回；否则返回nullptr
Cache::Handle* NvmTable::GetEntryFromCache(
  ...
  // 根据key值，从block_cache中查找对应的value(即block的内容)
  // 1.如果缓存中不存在对应值，则cache_handle返回nullptr
  ...
  // 2.如果缓存中存在对应值，则cache_handle返回对应的handle，同时增加一些统计信息
...
// func: 将一个key和对应的value插入cache中
template <typename TBlocklike>
Status NvmTable::InsertEntryToCache(
    const CacheTier& cache_tier, Cache* block_cache, const Slice& key,
    const Cache::CacheItemHelper* cache_helper,
...
void NvmTable::SetupBaseCacheKey(const TableProperties* properties,
...
CacheKey NvmTable::GetCacheKey(const OffsetableCacheKey& base_cache_key,
...
Status NvmTable::Open(
  ...
  Rep* rep = new NvmTable::Rep(ioptions, env_options, table_options,
  ...
  // Read metaindex
  std::unique_ptr<NvmTable> new_table(
      new NvmTable(rep, block_cache_tracer));
  ...
  TEST_SYNC_POINT_CALLBACK(
      "NvmTable::Open::ForceNullTablePrefixExtractor",
...
Status NvmTable::PrefetchTail(
    const ReadOptions& ro, RandomAccessFileReader* file, uint64_t file_size,
    bool force_direct_prefetch, TailPrefetchStats2* tail_prefetch_stats,
  ...
  TEST_SYNC_POINT_CALLBACK("NvmTable::Open::TailPrefetchLen",
                           &tail_prefetch_size);
...
Status NvmTable::ReadPropertiesBlock(
...
Status NvmTable::ReadRangeDelBlock(
...
// Unused 我们没有IndexBlock，所以无需预热...
Status NvmTable::PrefetchIndexAndFilterBlocks(
    const ReadOptions& ro, FilePrefetchBuffer* prefetch_buffer,
    InternalIterator* meta_iter, NvmTable* new_table, bool prefetch_all,
...
//     // Create函数和BlockBasedTable的耦合，这里直接返回Status::OK
//     s = Status::OK();
...
void NvmTable::SetupForCompaction() {
...
std::shared_ptr<const TableProperties> NvmTable::GetTableProperties()
    const {
  return rep_->table_properties;
}

size_t NvmTable::ApproximateMemoryUsage() const {
  size_t usage = 0;
  if (rep_) {
...
Status NvmTable::ReadMetaIndexBlock(
    const ReadOptions& ro, FilePrefetchBuffer* prefetch_buffer,
...
// func: 基于一个block的唯一id作为key，去cache中查找它是否存在于cache。如果
// 存在，将结果句柄、value、Cache对象 放到CachableEntry对象block中
template <typename TBlocklike>
Status NvmTable::GetDataBlockFromCache(
  ...
  // 1. 在未压缩block cache中查找。
  // 1.1 如果找到，将缓存句柄存放于cache_handle
  if (block_cache != nullptr) {
    ...
    cache_handle = GetEntryFromCache(
        ...
        priority);
    if (cache_handle != nullptr) {
      // 1.2 如果找到了，将指向cache中缓存的value的指针存入CachableEntry中
      // 注意，此处的TBlocklike是Block类 也就是说，缓存里值是Block，是一个
      // 包含block起始地址和block size的类，而不是真正的block内容。
  ...
  // 2.在已压缩block cache中查找。如果找到，将缓存句柄存放于block_cache_compressed_handle
  assert(!cache_key.empty());
  BlockContents contents;
  ...
  // 3.将已压缩block内容解压，将其中内容存入contents
  ...
  // data block type.
  // 4.根据contents内容构造一个Block对象作为value，然后将其存入未压缩的cache中。
  if (s.ok()) {
    ...
    if (block_cache != nullptr && block_holder->own_bytes() &&
      ...
      // 5.将conents中的内容存入CachableEntry中。
      if (s.ok()) {
...
template <typename TBlocklike>
Status NvmTable::PutDataBlockToCache(
    const Slice& cache_key, Cache* block_cache, Cache* block_cache_compressed,
...
std::unique_ptr<FilterBlockReader> NvmTable::CreateFilterBlockReader(
  ...
  // // 我们没有FilterBlock，所以这里默认返回nullptr
  // switch (filter_type) {
...
InternalIteratorBase<IndexValue>* NvmTable::NewIndexIterator(
...
template <>
DataBlockIter* NvmTable::InitBlockIterator<DataBlockIter>(
...
template <>
IndexBlockIter* NvmTable::InitBlockIterator<IndexBlockIter>(
    const Rep* rep, Block* block, BlockType block_type,
...
// 如果contents 为 nullptr，则此函数查找块缓存以查找 handle 引用的数据块，并在必要时从磁盘读取该块。
// 如果contents不为nullptr，它会跳过缓存查找和磁盘读取，因为调用者已经读取了它。 在这两种情况下，如果 ro.fill_cache 为真，它会将块插入到块缓存中。
template <typename TBlocklike>
Status NvmTable::MaybeReadBlockAndLoadToCache(
  ...
  // 1.获得两个cache 分别是未压缩的cache和压缩后的cache
  ...
  if (block_cache != nullptr || block_cache_compressed != nullptr) {
    ...
    // 2.1基于本block的一些特性，生成唯一key(包括db name、file number、这个block的大小即offset)
    key_data = GetCacheKey(rep_->base_cache_key, handle);
    key = key_data.AsSlice();

    if (!contents) {
      // 2.2 根据key的值，去cache对象中获得具体的block内容，并将value放到block_entry中
      StopWatch sw(rep_->ioptions.clock, rep_->ioptions.stats, DB_DATA_CACHE_GET);
      ...
      // 2.3 如果block_entry的value不为空，则说明从cache中已经得到了value
    ...
    // file.
    // 3. 如果从cache中找不到本block对应的内容，那么需要去磁盘读取
    // Question: RocksDB到底怎么知道去读磁盘文件的第几个block？
    // Answer: 在handle中存储了当前要查找的block的offset和size。去文件读时，就根据这个信息得到具体的block
    if (block_entry->GetValue() == nullptr &&
      ...
      if (!contents) {
        ...
        // 3.根据handle去文件读，读取到的内容放到raw_block_contents中 通过ReadBlockContents方法
        BlockFetcher block_fetcher(
      ...
      if (s.ok()) {
        ...
        // block to the cache.
        // 4.把block添加到cache中 其中contents中包含了从磁盘读到的数据 
        // 添加cache之后，把句柄存入block_entry中
...
//func: 根据handle中的block的offset和size，从cache或磁盘中获得一个具体的block内容，结果存在block_entry这个句柄中
template <typename TBlocklike>
Status NvmTable::RetrieveBlock(
    FilePrefetchBuffer* prefetch_buffer, const ReadOptions& ro,
    const BlockHandle& handle, const UncompressionDict& uncompression_dict,
    CachableEntry<TBlocklike>* block_entry, BlockType block_type,
    GetContext* get_context, BlockCacheLookupContext* lookup_context,
  ...
  Status s;
  // 1.首先从cache和磁盘中查找该block。并将block的句柄放在block_entry中返回
  if (use_cache) {
  ...
  std::cout<<"block_entry == nullptr"<<std::endl;
  assert(block_entry->IsEmpty());
...

  {
    ...
    // 2. 如果上面没查到，那么还要在磁盘中再进行一次查询
    // 这次查询的区别是，从磁盘查到之后，并不会将其加入cache中
    // 所以block_entry中仅仅存储了value，而没有存储cache和cache_handle。
    // 此时block_entry的管理的value的生命周期是自己控制的，而不是cache控制。
    s = ReadBlockFromFile(
...
template Status NvmTable::RetrieveBlock<BlockContents>(
    FilePrefetchBuffer* prefetch_buffer, const ReadOptions& ro,
    const BlockHandle& handle, const UncompressionDict& uncompression_dict,
...
template Status NvmTable::RetrieveBlock<ParsedFullFilterBlock>(
    FilePrefetchBuffer* prefetch_buffer, const ReadOptions& ro,
    const BlockHandle& handle, const UncompressionDict& uncompression_dict,
...
template Status NvmTable::RetrieveBlock<Block>(
    FilePrefetchBuffer* prefetch_buffer, const ReadOptions& ro,
    const BlockHandle& handle, const UncompressionDict& uncompression_dict,
...
template Status NvmTable::RetrieveBlock<UncompressionDict>(
    FilePrefetchBuffer* prefetch_buffer, const ReadOptions& ro,
    const BlockHandle& handle, const UncompressionDict& uncompression_dict,
...
NvmTable::PartitionedIndexIteratorState::PartitionedIndexIteratorState(
    const NvmTable* table,
    UnorderedMap<uint64_t, CachableEntry<Block>>* block_map)
    : table_(table), block_map_(block_map) {}

InternalIteratorBase<IndexValue>*
NvmTable::PartitionedIndexIteratorState::NewSecondaryIterator(
    const BlockHandle& handle) {
...
bool NvmTable::PrefixRangeMayMatch(
...
bool NvmTable::PrefixExtractorChanged(
...
// NvmTable不需要Iterator的存在......
InternalIterator* NvmTable::NewIterator(
...
FragmentedRangeTombstoneIterator* NvmTable::NewRangeTombstoneIterator(
...
bool NvmTable::FullFilterKeyMayMatch(
...
void NvmTable::FullFilterKeysMayMatch(
...
Status NvmTable::ApproximateKeyAnchors(const ReadOptions& read_options,
...
// Unused MultiGet真正的逻辑在sync_and_async那个.h文件中
void NvmTable::MultiGet (const ReadOptions& read_options, const MultiGetRange* mget_range,
 const SliceTransform* prefix_extractor, bool skip_filters) {

 }

// Unused 根据key查找在NvmTable中无用
Status NvmTable::Get(const ReadOptions& read_options, const Slice& key,
                            GetContext* get_context,
                            const SliceTransform* prefix_extractor,
...
// func: 根据offset从磁盘中获取value信息 这是我们需要重点设计的函数
// 函数的入参包括GetContext*和Slice*，这并不冲突。前者用来统计get过程中的统计信息，后者返回给更上层
// 上一层直接调用这里的Get，把get_context传给它。
// filter的逻辑不应该在这一层获得，而是在上一层对整个B-Tree index索引的内容计算，所以filter逻辑可以删掉
Status NvmTable::Get2(const ReadOptions& read_options, const uint64_t& offset,
  ...
  // 1.根据offset获得BlockHandle和block_offset
  // 注意，block_offset中的size不应该包括最后那5byte......
  uint32_t block_offset;
  BlockHandle handle = GetBlockHandleFromOffset(read_options,offset,block_offset);

  bool does_referenced_key_exist = false;
  uint64_t referenced_data_size = 0;
  Status tmp_status;
  // 2.原NewDataBlockIterator内部逻辑 主要是获取handle指向的Data Block

  const UncompressionDict& dict = UncompressionDict::GetEmptyDict();

  CachableEntry<Block> block;
  // 从磁盘或者cache中获取block，并将句柄存放于block中
  s = RetrieveBlock(nullptr, read_options, handle, dict, &block, BlockType::kData,
                    get_context, nullptr, /* for_compaction */false,
                    /* use_cache */ true, /* wait_for_cache */ true,
                    /* async_read*/false);
  // 微吐槽:这里的assert失败是因为RetriveBlock里面的层层进去不知道第多少个函数的checksum校验失败造成的
  // 但是校验失败那里明明也返回了一个错误的Status
  // Question: 所以为什么不在这一行代码直接对Status进行处理呢？就好比Golang中的if err != nil 一样...
  assert(block.GetValue() != nullptr);
  const bool block_contents_pinned =
        block.IsCached() ||
        (!block.GetValue()->own_bytes() && rep_->immortal_table);

  // RetrieveBlock返回的CachableEntry可能来自Cache也可能不来自cache
  // 如果不来自cache，且option中有设置，那么就将其再加入cache
  if (!block.IsCached()) {
  ...
  // 3.根据data block的content和具体的block_offset，获得真正的value返回
  Block* block2 = block.GetValue();
  const char* start = block2->data()+block_offset;
  *value = GetLengthPrefixedSlice(start);
...
Status NvmTable::Prefetch(const Slice* const begin,
...
Status NvmTable::VerifyChecksum(const ReadOptions& read_options,
...
Status NvmTable::VerifyChecksumInBlocks(
    const ReadOptions& read_options,
...
BlockType NvmTable::GetBlockTypeForMetaBlockByName(
...
Status NvmTable::VerifyChecksumInMetaBlocks(
...
bool NvmTable::TEST_BlockInCache(const BlockHandle& handle) const {
  assert(rep_ != nullptr);
...
bool NvmTable::TEST_KeyInCache(const ReadOptions& options,
                                      const Slice& key) {
...
uint64_t NvmTable::ApproximateDataOffsetOf(
...
uint64_t NvmTable::GetApproximateDataSize() {
...
uint64_t NvmTable::ApproximateOffsetOf(const Slice& key,
                                              TableReaderCaller caller) {
...
uint64_t NvmTable::ApproximateSize(const Slice& start, const Slice& end,
                                          TableReaderCaller caller) {
...
Status NvmTable::DumpTable(WritableFile* out_file) {
  WritableFileStringStreamAdapter2 out_file_wrapper(out_file);
...
Status NvmTable::DumpIndexBlock(std::ostream& out_stream) {
...
Status NvmTable::DumpDataBlocks(std::ostream& out_stream) {
  std::unique_ptr<InternalIteratorBase<IndexValue>> blockhandles_iter(
      NewIndexIterator(ReadOptions(), /*need_upper_bound_check=*/false,
                       /*input_iter=*/nullptr, /*get_context=*/nullptr,
                       /*lookup_contex=*/nullptr));
...
void NvmTable::DumpKeyValue(const Slice& key, const Slice& value,
...
TBlockIter* NvmTable::NewDataBlockIterator(const ReadOptions& ro,
                                                  CachableEntry<Block>& block,
