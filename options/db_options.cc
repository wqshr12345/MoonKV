    ...
    db_immutable_options_type_info = {
        ...
        {"max_log_file_size",
         {offsetof(struct ImmutableDBOptions, max_log_file_size),
          ...
          OptionTypeFlags::kNone}},
        {"db_nvm_dir",
         {offsetof(struct ImmutableDBOptions, db_nvm_dir), OptionType::kString,
          OptionVerificationType::kNormal, OptionTypeFlags::kNone}},
        {"db_log_dir",
...
ImmutableDBOptions::ImmutableDBOptions(const DBOptions& options)
  ...
  if (db_nvm_dir == "" && db_paths.size() > 0) {
    db_nvm_dir = db_paths[0].path + "/nvm";
  }
  // PATH:在这里对ImmutableDBOptions.db_paths添加更多的文件路径，使不同层的文件存储在不同的文件路径下
  if (db_paths.size() == 1) {
    db_paths.emplace_back(db_nvm_dir, 0);
    // table文件不再做路径划分
  }
}
...
void ImmutableDBOptions::Dump(Logger* log) const {
  ...
  ROCKS_LOG_HEADER(log, "         Options.create_missing_column_families: %d",
                   create_missing_column_families);
  ROCKS_LOG_HEADER(log, "                             Options.db_nvm_dir: %s",
                   db_nvm_dir.c_str());
