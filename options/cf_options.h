...
struct ImmutableCFOptions {
 public:
  ...
  int max_gc_compactions;

  double gc_multiple;

  double gc_garbage_threshold;

  int gc_range_tables_threshold;

  double pick_table_threshold;

  double pick_table_max_multiple;

  bool enable_vertical_compaction;

  double vc_merge_threshold;

  double vc_merge_threshold_nostart;

  int vertical_start_level;

  int vertical_min_involve_levels;

  uint32_t pick_index_min_table_numbers;

  uint64_t choose_guard_interval;

  uint32_t neighbor_guard_ratio;

  bool enable_estimate_seek;

  int estimate_interval;

  int estimate_threshold;
  ...
  // 本TableFactory默认为NvmTableFactory，用于
  // 1.NvmPartiitonBuilder内部建立NvmTableBuilder对象
  // 2.NvmPartition内部建立NvmTable对象，并放入cache
