...
// 1. BlockBasedTable::GetFilter
// 2. BlockBasedTable::GetUncompressedDict。
// 3. BlockBasedTable::MaybeReadAndLoadToCache。 （跟踪对数据、索引和范围删除块的访问。）
// 4. BlockBasedTable::Get。 （跟踪引用的键以及引用的键是否存在于获取的数据块中。）
// 5. BlockBasedTable::MultiGet。 （跟踪引用的键以及引用的键是否存在于获取的数据块中。）
// 上下文创建于：
// 1. BlockBasedTable::Get。 (kUserGet)
// 2. BlockBasedTable::MultiGet。 (kUserMGet)
// 3. BlockBasedTable::NewIterator。 （kUserIterator、kCompaction 或外部 SST 摄取调用此函数。）
// 4. BlockBasedTable::Open。 (kPrefetch)
// 5. 索引/过滤器::CacheDependencies。 (kPrefetch)
// 6. BlockBasedTable::ApproximateOffsetOf。 (kCompaction 或
// kUserApproximateSize).
