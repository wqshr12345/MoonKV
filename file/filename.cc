...
static const std::string kRocksDbTFileExt = "table";
static const std::string kRocksDbIndex = "index";
...
std::string MakeIndexFileName(uint64_t number) {
  return MakeFileName(number, kRocksDbIndex.c_str());
}

std::string MakeIndexFileName(const std::string& path, uint64_t number){
  return path + "/" + MakeFileName(number, kRocksDbIndex.c_str());
}
...
bool ParseFileName(const std::string& fname, uint64_t* number,
  ...
  if (rest == "IDENTITY") {
    ...
    *type = is_temp_file ? kTempFile : kOptionsFile;
    ...
    } else if (suffix == Slice(kRocksDbIndex) ||
